...
const iconDirectory = path.join(root, 'public', 'img', 'items', 'vehicles');
const outputPath = path.resolve(process.argv[2]
  ?? path.join(os.tmpdir(), 'minethings-vehicle-icons.png'));
const icons = fs.readdirSync(iconDirectory)
  .filter((filename) => /^vehicle-\d+\.svg$/u.test(filename))
...
try {
  ...
  </style></head><body><main>${icons.map((icon) => `<article><h2>${icon.filename.slice(8, -4)} · ${icon.title}</h2><div class="samples"><div class="sample"><img class="large" src="${icon.src}"><img class="small" src="${icon.src}"></div><div class="sample dark"><img class="large" src="${icon.src}"><img class="small" src="${icon.src}"></div></div></article>`).join('')}</main></body></html>`);
