...
<p>If you'd like to deposit your gold at a bank and earn interest, use this form to apply.</p>
<?
if (isset($message)) echo '<div class="error">'.$message.'</div>';
echo $form->create('', array('action' => 'deposit_application'));
$update = 'UpdateDepositInfo($("Principle").value, $("Duration").value, $("Interest").value, $("Return"), $("InterestEarned"));';
...
echo '<div>Return: <div id="Return" style="display:inline"></div>g</div>';
echo '<div>Interest Earned: <div id="InterestEarned" style="display:inline"></div>g</div>';
