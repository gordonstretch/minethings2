...
function launchVehicleAtCreature(store, playerId, creatureId, vehicleId, now) {
  const creature = store.database.prepare(
    'SELECT route_id FROM world_creatures WHERE id = ?'
  ).get(creatureId);
  assert.ok(creature, 'the route creature should exist before departure');
  const journey = store.sendVehicle(playerId, vehicleId, creature.route_id, now);
  ...
  assert.ok(pursuit, 'a PvE-enabled departure should launch a physical route hunt');
  assert.equal(pursuit.initiator, 'hunter');
  assert.ok(journey.huntsLaunched > 0);
  return {
    ...journey,
    pursuitId: Number(pursuit.id),
    encounterAt: Number(pursuit.encounter_at),
    encounterLocation: Number(pursuit.encounter_location)
  };
}
...
test('counts contextual extraction links and operations available in the current city', (context) => {
  ...
  const nonFieldCityId = Number(store.database.prepare(`
    ...
    WHERE NOT EXISTS (
      ...
      AND catalog_cities.id NOT IN (?, ?)
    ORDER BY catalog_cities.id LIMIT 1
  `).get(oilFieldCityId, oilFieldCityId, capitalCityId).id);
...
test('creates the findings and restless-wreck ledgers on a fresh database', (context) => {
  ...
  const mineShiftColumn = store.database.prepare('PRAGMA table_info(mines)')
    .all().find((column) => column.name === 'shift_mask');
  assert.ok(mineShiftColumn);
  assert.equal(Number(mineShiftColumn.dflt_value), 7);
  ...
  for (const index of ['finding_events_unsent_day', 'finding_events_player_time',
    ...
    'ghost_vehicles_source_creature', 'thief_bases_map_recent']) {
    assert.ok(store.database.prepare(
  ...
  `).get().value_json), 100);
  assert.ok(store.database.prepare('PRAGMA table_info(ghost_vehicles)')
    .all().some((column) => column.name === 'source_creature_id'));
  assert.equal(store.database.prepare('PRAGMA foreign_key_check').get(), undefined);
});

test('migrates v141 mines onto the three-shift robot roster', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-mine-shifts-'));
  const databaseFile = path.join(directory, 'mine-shifts.sqlite');
  let store = new SqliteStore(databaseFile);
  store.seedCatalog(catalog);
  const player = store.addPlayer(createPlayer(
    'Shift Migration', '', 'hash', catalog, 1000, () => 0.5
  ));
  store.database.exec(`
    ALTER TABLE mines DROP COLUMN shift_mask;
  ...
  assert.equal(store.database.prepare('PRAGMA user_version').get().user_version, 142);
  const column = store.database.prepare('PRAGMA table_info(mines)')
    .all().find((entry) => entry.name === 'shift_mask');
  assert.ok(column);
  assert.equal(Number(column.dflt_value), 7);
  assert.equal(store.playerById(player.id).mines[0].shiftMask, 7);
});

test('migrates the legacy ore-thief base into Gallego and creates regional operations',
  (context) => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-regional-thieves-'));
    const databaseFile = path.join(directory, 'game.sqlite');
    let store = new SqliteStore(databaseFile);
    context.after(() => {
    ...
    store.seedCatalog(catalog);
    store.ensureWorldMaps(1000);
    const gallego = store.database.prepare(
      "SELECT id FROM world_maps WHERE slug = 'gallego'"
    ).get();
    const legacyBase = store.database.prepare(`
      SELECT id FROM thief_bases WHERE map_id = ? ORDER BY id DESC LIMIT 1
    `).get(gallego.id);
    store.database.prepare(`
      UPDATE thief_bases SET distance = 333, buckets = 12345 WHERE id = ?
    `).run(legacyBase.id);
    store.database.prepare('DELETE FROM thief_bases WHERE id != ?').run(legacyBase.id);
    store.database.prepare(`
      DELETE FROM schema_migrations WHERE name = 'regional-ore-thief-operations-v1'
    `).run();
    store.database.exec(`
      DROP INDEX thief_bases_map_recent;
      ALTER TABLE thief_bases DROP COLUMN map_id;
    ...
    const migrated = store.database.prepare(`
      SELECT map_id, distance, buckets FROM thief_bases WHERE id = ?
    `).get(legacyBase.id);
    assert.deepEqual({ ...migrated }, {
      map_id: gallego.id, distance: 333, buckets: 12345
    });
    assert.equal(store.database.prepare(`
      SELECT COUNT(DISTINCT map_id) AS count FROM thief_bases
    `).get().count, 7);
    assert.equal(store.database.prepare(`
      SELECT COUNT(DISTINCT catalog_cities.map_id) AS count
      FROM catalog_routes
      JOIN catalog_cities ON catalog_cities.id = catalog_routes.city1_id
      WHERE catalog_routes.type = 2
...
test('migrates v109 databases to the starter welcome-pack setting without backfilling miners', (context) => {
  ...
  store.database.prepare(
    'DELETE FROM inventory WHERE player_id = ? AND item_id = ?'
  ).run(existing.id, LEGACY_STARTER_WELCOME_PACK.dwarfItemId);
  for (const gadgetItemId of LEGACY_STARTER_WELCOME_PACK.gadgetItemIds) {
    store.database.prepare(
      'DELETE FROM inventory WHERE player_id = ? AND item_id = ?'
    ).run(existing.id, gadgetItemId);
  }
  store.database.exec(`
  ...
  assert.equal(unchanged.inventory[LEGACY_STARTER_WELCOME_PACK.dwarfItemId], undefined);
  for (const gadgetItemId of LEGACY_STARTER_WELCOME_PACK.gadgetItemIds) {
    assert.equal(unchanged.inventory[gadgetItemId], undefined);
  }
  for (const { itemId } of LEGACY_STARTER_WELCOME_PACK.itemGrants) {
...
test('upgrades legacy starter packs without backfilling miners', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-starter-dwarf-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const existing = store.addPlayer(createPlayer(
    'Pre-Dwarf Miner', '', 'hash', catalog, 1000, () => 0.5
  ));
  store.database.prepare(
    'DELETE FROM inventory WHERE player_id = ? AND item_id = ?'
  ).run(existing.id, LEGACY_STARTER_WELCOME_PACK.dwarfItemId);
  ...
  const legacyPack = {
    vehicleItemId: LEGACY_STARTER_WELCOME_PACK.vehicleItemId,
    cryptoTypeId: 2,
    cryptoQuantity: 7
  };
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = ? WHERE key = 'starter_welcome_pack'
  `).run(JSON.stringify(legacyPack));
  store.close();
  ...
  `).get().value_json), {
    ...legacyPack,
    dwarfItemId: LEGACY_STARTER_WELCOME_PACK.dwarfItemId,
    rentalMineTypeIds: LEGACY_STARTER_WELCOME_PACK.rentalMineTypeIds,
    casinoVoucherCryptoTypeId: LEGACY_STARTER_WELCOME_PACK.casinoVoucherCryptoTypeId,
    casinoVoucherQuantity: LEGACY_STARTER_WELCOME_PACK.casinoVoucherQuantity,
    gadgetItemIds: LEGACY_STARTER_WELCOME_PACK.gadgetItemIds,
    itemGrants: LEGACY_STARTER_WELCOME_PACK.itemGrants,
    safeTravelKitItemId: LEGACY_STARTER_WELCOME_PACK.safeTravelKitItemId
  });
  const unchanged = store.playerById(existing.id);
  assert.equal(unchanged.inventory[
    LEGACY_STARTER_WELCOME_PACK.dwarfItemId
  ], undefined);
  for (const gadgetItemId of LEGACY_STARTER_WELCOME_PACK.gadgetItemIds) {
    assert.equal(unchanged.inventory[gadgetItemId], undefined);
  }
  ...
  const migration = store.database.prepare(`
    ...
    WHERE name = 'starter-green-dwarf-v1'
  `).get();
  assert.ok(migration);
  ...
  const expansionMigration = store.database.prepare(`
    SELECT applied_at, details_json FROM schema_migrations
    WHERE name = 'starter-rentals-casino-voucher-v1'
  `).get();
  assert.ok(expansionMigration);
  assert.deepEqual(JSON.parse(expansionMigration.details_json), {
    changedFields: [
      'rentalMineTypeIds', 'casinoVoucherCryptoTypeId', 'casinoVoucherQuantity'
    ]
  });
  const gadgetMigration = store.database.prepare(`
    SELECT applied_at, details_json FROM schema_migrations
    WHERE name = 'starter-council-gadgets-v1'
  `).get();
  assert.ok(gadgetMigration);
  assert.deepEqual(JSON.parse(gadgetMigration.details_json), {
    gadgetItemIds: LEGACY_STARTER_WELCOME_PACK.gadgetItemIds,
    changed: true
  });
  const supplyMigration = store.database.prepare(`
    SELECT applied_at, details_json FROM schema_migrations
    WHERE name = 'starter-oil-field-and-m80-supplies-v1'
  `).get();
  assert.ok(supplyMigration);
  assert.deepEqual(JSON.parse(supplyMigration.details_json), {
    itemGrants: LEGACY_STARTER_WELCOME_PACK.itemGrants,
    changed: true
  });
...
test('migrates v130 mines to durable rate-aware schedules', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-mine-schedule-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  const player = store.addPlayer(
    createPlayer('Schedule Migrant', '', 'hash', catalog, 1000, () => 0.5)
  );
  store.close();

  const legacy = new DatabaseSync(databaseFile);
  legacy.exec(`
    ALTER TABLE mines DROP COLUMN cycle_interval_ms;
...
test('migrates v131 gadget ownership and catalog entries to automation machinery', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-gadget-automation-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const player = store.addPlayer(
    createPlayer('Automation Migrant', '', 'hash', catalog, 1000, () => 0.5)
  );
  store.database.prepare(
    'INSERT INTO player_gadgets (player_id, gadget_id, expires_at) VALUES (?, 6, 9000)'
  ).run(player.id);
  store.database.exec(`
    UPDATE catalog_gadgets SET name = 'calculator', behavior_key = 'calculator',
      display_name = 'Calculator' WHERE id = 6;
    UPDATE catalog_items SET name = 'Titanium Calculator', rarity = 4,
      gold_value_units = 2000000 WHERE id = 276;
  ...
  `).get() }, { behaviorKey: 'autolister', displayName: 'Autolister' });
  assert.deepEqual({ ...store.database.prepare(`
    SELECT name, rarity, gold_value_units AS goldValueUnits FROM catalog_items WHERE id = 276
  `).get() }, { name: 'Tungsten Autolister', rarity: 5, goldValueUnits: 14010000 });
  assert.equal(store.database.prepare(`
    SELECT expires_at FROM player_gadgets WHERE player_id = ? AND gadget_id = 6
  `).get(player.id).expires_at, 9000);
  assert.ok(store.database.prepare(`
    SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'gadget_automations'
  `).get());
  assert.ok(store.database.prepare(`
    SELECT 1 FROM sqlite_master
    WHERE type = 'trigger' AND name = 'live_update_gadget_automations_update'
  `).get());
});

test('migrates v132 automation settings into ten-task queues and installs Legendary activators',
  (context) => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-gadget-queues-'));
    const databaseFile = path.join(directory, 'game.sqlite');
    let store = new SqliteStore(databaseFile);
    context.after(() => {
      store.close();
    ...
    store.seedCatalog(catalog);
    const player = store.addPlayer(
      createPlayer('Queue Migrant', '', 'hash', catalog, 1000, () => 0.5)
    );
    store.database.prepare(`
      INSERT INTO gadget_automations
        (player_id, gadget_id, city_id, configuration_json, interval_ms,
         next_run_at, last_status, enabled, created_at, updated_at)
      VALUES (?, 6, ?, ?, 900000, 3000, '', 1, 2000, 2000)
    `).run(player.id, player.cityId, JSON.stringify({ itemId: 1, markupPercent: 25 }));
    store.database.exec(`
      DELETE FROM catalog_gadget_items WHERE id BETWEEN 40 AND 43;
      DELETE FROM catalog_items WHERE id BETWEEN 1593 AND 1596;
      DELETE FROM catalog_items WHERE id BETWEEN 1589 AND 1592;
      DELETE FROM catalog_settings WHERE key = 'gadget_automation_max_tasks';
      UPDATE catalog_settings SET value_json = '[0,0.5,3.5,20,120,480]'
        WHERE key = 'gadget_lifespan_days';
    ...
    `).get(player.id).configuration_json), {
      tasks: [{
        cityId: player.cityId, mineTypeId: catalog.byId.get(1).mineTypeId, markupPercent: 25
      }], cursor: 0
    });
    assert.equal(JSON.parse(store.database.prepare(`
      SELECT value_json FROM catalog_settings WHERE key = 'gadget_automation_max_tasks'
    `).get().value_json), 10);
    assert.equal(JSON.parse(store.database.prepare(`
      SELECT value_json FROM catalog_settings WHERE key = 'gadget_lifespan_days'
    `).get().value_json)[6], 1920);
    assert.deepEqual(store.database.prepare(`
      SELECT id, name, rarity, repaired_item_id AS repairedItemId, gold_value_units AS value
      FROM catalog_items WHERE id BETWEEN 1589 AND 1596 ORDER BY id
    `).all().map((row) => ({ ...row })), [
      { id: 1589, name: 'Legendary Autoloader', rarity: 6, repairedItemId: null, value: 84045000 },
      { id: 1590, name: 'Legendary Autolister', rarity: 6, repairedItemId: null, value: 84045000 },
      { id: 1591, name: 'Legendary Automaker', rarity: 6, repairedItemId: null, value: 84045000 },
      { id: 1592, name: 'Legendary Automelder', rarity: 6, repairedItemId: null, value: 84045000 },
      { id: 1593, name: 'Damaged Legendary Autoloader', rarity: 6, repairedItemId: 1589, value: 84035000 },
      { id: 1594, name: 'Damaged Legendary Autolister', rarity: 6, repairedItemId: 1590, value: 84035000 },
      { id: 1595, name: 'Damaged Legendary Automaker', rarity: 6, repairedItemId: 1591, value: 84035000 },
      { id: 1596, name: 'Damaged Legendary Automelder', rarity: 6, repairedItemId: 1592, value: 84035000 }
    ]);
    assert.deepEqual(store.database.prepare(`
      SELECT id, gadget_id AS gadgetId, item_id AS itemId
      FROM catalog_gadget_items WHERE id BETWEEN 40 AND 43 ORDER BY id
    `).all().map((row) => ({ ...row })), [
      { id: 40, gadgetId: 5, itemId: 1589 },
      { id: 41, gadgetId: 6, itemId: 1590 },
      { id: 42, gadgetId: 11, itemId: 1591 },
      { id: 43, gadgetId: 12, itemId: 1592 }
    ]);
  });

test('migrates item-specific Autolister queues to distinct Thing-type tasks', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-autolister-types-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const player = store.addPlayer(
    createPlayer('Type Queue Migrant', '', 'hash', catalog, 1000, () => 0.5)
  );
  const first = catalog.items.find((item) => item.marketableId && !item.damaged);
  const second = catalog.items.find((item) => item.marketableId && !item.damaged
    && item.id !== first.id && item.mineTypeId === first.mineTypeId);
  const third = catalog.items.find((item) => item.marketableId && !item.damaged
    && item.mineTypeId !== first.mineTypeId);
  ...
  store.database.prepare(`
    INSERT INTO gadget_automations
      (player_id, gadget_id, city_id, configuration_json, interval_ms,
       next_run_at, last_status, enabled, created_at, updated_at)
    VALUES (?, 6, ?, ?, 900000, 3000, '', 1, 2000, 2000)
  `).run(player.id, player.cityId, JSON.stringify({ tasks: [
    { cityId: player.cityId, itemId: first.id, markupPercent: 10 },
    { cityId: player.cityId, itemId: second.id, markupPercent: 20 },
    { cityId: player.cityId, itemId: third.id, markupPercent: 30 }
  ], cursor: 1 }));
  store.database.exec('PRAGMA user_version = 135');
  store.close();
  ...
  assert.deepEqual(JSON.parse(store.database.prepare(`
    SELECT configuration_json FROM gadget_automations
    WHERE player_id = ? AND gadget_id = 6
  `).get(player.id).configuration_json), { tasks: [
    { cityId: player.cityId, markupPercent: 20, mineTypeId: first.mineTypeId },
    { cityId: player.cityId, markupPercent: 30, mineTypeId: third.mineTypeId }
  ], cursor: 0 });
});
...
test('records scoped live updates for application and direct database writes', (context) => {
  ...
  store.database.prepare("DELETE FROM live_update_events WHERE scope = 'topic:stats'").run();
  let wakeCount = 0;
  ...
  assert.ok(events.some((event) => event.scope === `player:${player.id}`));
  const catalogEvents = store.liveUpdatesAfter(revision, 2000, new Set(['topic:catalog']));
  assert.ok(catalogEvents.length > 0);
  assert.ok(catalogEvents.every((event) => event.scope === 'topic:catalog'),
    'SSE catch-up can use its scope index instead of hydrating unrelated events');
  const playerEvents = store.liveUpdatesAfter(
    revision, 2000, new Set([`player:${player.id}`]));
  assert.ok(playerEvents.length > 0);
  assert.ok(playerEvents.every((event) => event.scope === `player:${player.id}`));
  ...
  assert.equal(store.database.prepare(
    "SELECT COUNT(*) AS count FROM live_update_events WHERE scope = 'topic:stats'"
  ).get().count, 1, 'rapid aggregate mutations coalesce into one stats signal');
  assert.deepEqual(JSON.parse(store.database.prepare(`
    SELECT details_json FROM schema_migrations WHERE name = 'live-stats-coalescing-v1'
  `).get().details_json), { minimumIntervalMs: 30000 });

  const populated = store.playerById(player.id, 3000, { settle: false });
  const localInventory = populated.inventoryByCity[populated.cityId] ??= {};
  for (const item of catalog.items.slice(0, 40)) localInventory[item.id] = 1;
  store.savePlayer(populated);
  revision = store.latestLiveUpdateId();
  const oneItemChanged = store.playerById(player.id, 3001, { settle: false });
  oneItemChanged.inventoryByCity[oneItemChanged.cityId][catalog.items[0].id] += 1;
  store.savePlayer(oneItemChanged);
  const privateEvents = store.liveUpdatesAfter(revision)
    .filter((event) => event.scope === `player:${player.id}` && !event.eventType);
  assert.ok(privateEvents.length <= 4,
    `one inventory change created ${privateEvents.length} private live events`);
  assert.equal(store.database.prepare(`
    SELECT COUNT(*) AS count FROM inventory WHERE player_id = ?
  `).get(player.id).count, 40,
  'saving one changed item preserves the other inventory rows without rewriting them');
  unsubscribe();
});
...
test('rebases existing and new capacity to 500 with unique containers reaching 1000', (context) => {
  ...
  store.database.exec(`
    UPDATE players SET item_limit = 5000;
    UPDATE players SET item_limit = 5025 WHERE id = ${containerOwner.id};
    UPDATE catalog_containers SET capacity = CASE id WHEN 8 THEN 25 ELSE 50 END;
    UPDATE catalog_settings SET value_json = '5000' WHERE key = 'starter_item_limit';
  ...
  assert.equal(store.playerById(newcomer.id).baseItemLimit, 500);
...
test('uses original player-priced item listings, bids, and best-price FIFO fills', (context) => {
  ...
  assert.deepEqual(store.itemListingQuantities(savedSeller.id), {
    1: { [itemId]: 2 }
  });
  ...
  assert.deepEqual(store.itemListingQuantities(savedSeller.id), {});
  assert.deepEqual(store.itemListingQuantities(savedSecondSeller.id), {
    1: { [itemId]: 1 }
  });
  assert.equal(store.playerById(savedSecondSeller.id).inventory[itemId], 1);
  assert.equal(store.playerById(buyer.id).inventory[itemId], 3,
    'a completed purchase must deliver every bought item to the buyer');
  assert.equal(purchase.inventoryQuantity, 3);
  assert.equal(purchase.cityId, buyer.cityId);
...
test('reconciles legacy Wraith fittings with the equipment on the original wreck',
  (context) => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-wraith-loadout-'));
    const databaseFile = path.join(directory, 'wraith-loadout.sqlite');
    let store = new SqliteStore(databaseFile);
    store.seedCatalog(catalog);
    store.ensureWorldMaps(1000);
    context.after(() => {
    ...
    const administrator = store.addPlayer(createPlayer(
      'Wraith Loadout Admin', '', 'hash', catalog, 1000, () => 0.5
    ));
    const route = catalog.routes.find((entry) => entry.open
      && entry.type === catalog.settings.route_type_ids.land
      && entry.length > 0 && entry.city1Id !== entry.city2Id);
    assert.ok(route);
    const ghost = store.adminRaiseGhost(administrator.id, 'rider', route.id, 1, 2000);
    const mods = store.database.prepare(`
      SELECT catalog_mods.id FROM catalog_mods
      JOIN catalog_items ON catalog_items.id = catalog_mods.item_id
      WHERE catalog_items.rarity = 1 ORDER BY catalog_mods.id LIMIT 4
    `).all();
    const weapons = store.database.prepare(`
      SELECT catalog_weapons.id FROM catalog_weapons
      JOIN catalog_items ON catalog_items.id = catalog_weapons.item_id
      WHERE catalog_items.rarity = 1 ORDER BY catalog_weapons.id LIMIT 2
    `).all();
    assert.equal(mods.length, 4);
    assert.equal(weapons.length, 2);
    const insertMod = store.database.prepare(
      'INSERT INTO player_vehicle_mods (vehicle_id, mod_id) VALUES (?, ?)'
    );
    const insertWeapon = store.database.prepare(
      'INSERT INTO player_vehicle_weapons (vehicle_id, weapon_id) VALUES (?, ?)'
    );
    for (const mod of mods) insertMod.run(ghost.vehicleId, mod.id);
    for (const weapon of weapons) insertWeapon.run(ghost.vehicleId, weapon.id);
    store.database.prepare(`
      INSERT INTO ghost_wrecks
        (source_vehicle_id, source_player_id, vehicle_type_id, source_item_id,
         ...
         loadout_json, cause, wrecked_at, processed_at, outcome, ghost_id)
      SELECT 990002, ?, player_vehicles.vehicle_type_id, player_vehicles.item_id,
        'Recorded wreck', 'rider', ?, ?, 1, ?, 'legacy test', 1500, 2000,
        'risen', ?
      FROM player_vehicles WHERE player_vehicles.id = ?
    `).run(administrator.id, route.id, route.length / 2, JSON.stringify({
      mods: [{ id: mods[0].id }], weapons: [{ id: weapons[0].id }]
    }), ghost.id, ghost.vehicleId);
    store.database.prepare(
      "DELETE FROM schema_migrations WHERE name = 'proportional-ghost-rider-loadouts-v1'"
    ).run();
    store.close();

    store = new SqliteStore(databaseFile);
    assert.deepEqual(store.database.prepare(`
      SELECT mod_id FROM player_vehicle_mods WHERE vehicle_id = ? ORDER BY mod_id
    `).all(ghost.vehicleId).map((entry) => entry.mod_id), [mods[0].id]);
    assert.deepEqual(store.database.prepare(`
      SELECT weapon_id FROM player_vehicle_weapons WHERE vehicle_id = ? ORDER BY weapon_id
    `).all(ghost.vehicleId).map((entry) => entry.weapon_id), [weapons[0].id]);
    const migration = store.database.prepare(`
      SELECT details_json FROM schema_migrations
      WHERE name = 'proportional-ghost-rider-loadouts-v1'
    `).get();
    assert.deepEqual(JSON.parse(migration.details_json), {
      ghostRidersReconciled: 1,
      removedMods: 4,
      removedWeapons: 2,
      restoredMods: 1,
      restoredWeapons: 1
...
test('installs reduced Wraith base armour for existing active apparitions once', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-wraith-armour-'));
  const databaseFile = path.join(directory, 'wraith-armour.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
  ...
  store.ensureWorldMaps(1000);
  const administrator = store.addPlayer(createPlayer(
    'Wraith Armour Admin', '', 'hash', catalog, 1000, () => 0.5
  ));
  const route = catalog.routes.find((entry) => entry.open
    && entry.type === catalog.settings.route_type_ids.land
    && entry.length > 0 && entry.city1Id !== entry.city2Id);
  store.adminRaiseGhost(administrator.id, 'rider', route.id, 1, 2000);
  store.database.prepare("DELETE FROM catalog_settings WHERE key = 'wraith_base_armor_factor'")
    .run();
  store.database.prepare(
    "DELETE FROM schema_migrations WHERE name = 'wraith-base-armor-reduction-v1'"
  ).run();
  store.close();

  store = new SqliteStore(databaseFile);
  assert.equal(JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'wraith_base_armor_factor'
  `).get().value_json), 1 / 3);
  const migration = store.database.prepare(`
    SELECT details_json FROM schema_migrations
    WHERE name = 'wraith-base-armor-reduction-v1'
  `).get();
  assert.deepEqual(JSON.parse(migration.details_json), {
    settingInserted: 1, activeWraithsRebalanced: 1
  });

  store.database.prepare(`
    UPDATE catalog_settings SET value_json = '0.25'
    WHERE key = 'wraith_base_armor_factor'
  `).run();
  store.close();
  store = new SqliteStore(databaseFile);
  assert.equal(JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'wraith_base_armor_factor'
  `).get().value_json), 0.25, 'later operator tuning must be preserved');
});
...
test('delivers a bought Rifle to the buyer in the listing city', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const rifle = catalog.items.find((item) => item.name === 'Rifle');
  assert.ok(rifle);
  const seller = createPlayer('Rifle Seller', '', 'hash', catalog, 3000, () => 0.5);
  seller.inventory[rifle.id] = 1;
  seller.inventoryByCity[seller.cityId] = seller.inventory;
  const savedSeller = store.addPlayer(seller);
  const buyer = createPlayer('Rifle Buyer', '', 'hash', catalog, 3000, () => 0.5);
  const price = store.marketForItem(rifle.id, savedSeller.cityId).listingStartPrice;
  buyer.gold = price * 2;
  const savedBuyer = store.addPlayer(buyer);
  const listingId = store.placeSellOrder(
    savedSeller.id, rifle.id, price, 1, 4000
  );

  const purchase = store.buyListing(savedBuyer.id, listingId, 1, 5000);

  assert.equal(purchase.itemId, rifle.id);
  assert.equal(purchase.cityId, savedBuyer.cityId);
  assert.equal(purchase.inventoryQuantity, 1);
  assert.equal(store.playerById(savedBuyer.id).inventory[rifle.id], 1);
  assert.equal(store.playerById(savedSeller.id).inventory[rifle.id] ?? 0, 0);
});

test('trims a local listing as its stock leaves city inventory', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const rifle = catalog.items.find((item) => item.name === 'Rifle');
  assert.ok(rifle);
  const player = createPlayer('Departing Listing', '', 'hash', catalog, 3000, () => 0.5);
  player.inventory[rifle.id] = 2;
  player.inventoryByCity[player.cityId] = player.inventory;
  const saved = store.addPlayer(player);
  const price = store.marketForItem(rifle.id, saved.cityId).listingStartPrice;
  store.placeSellOrder(saved.id, rifle.id, price, 2, 4000);

  store.recycleInventory(saved.id, saved.cityId, rifle.id, 1, 5000);
  assert.equal(store.marketForItem(rifle.id, saved.cityId).listings[0].quantity, 1);

  store.recycleInventory(saved.id, saved.cityId, rifle.id, 1, 6000);
  assert.deepEqual(store.marketForItem(rifle.id, saved.cityId).listings, []);
});

test('removes stale local listings when an existing store opens', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-stale-listing-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const rifle = catalog.items.find((item) => item.name === 'Rifle');
  const player = createPlayer('Legacy Listing', '', 'hash', catalog, 3000, () => 0.5);
  player.inventory[rifle.id] = 1;
  player.inventoryByCity[player.cityId] = player.inventory;
  const saved = store.addPlayer(player);
  const price = store.marketForItem(rifle.id, saved.cityId).listingStartPrice;
  store.placeSellOrder(saved.id, rifle.id, price, 1, 4000);
  store.database.prepare(
    'DELETE FROM inventory WHERE player_id = ? AND city_id = ? AND item_id = ?'
  ).run(saved.id, saved.cityId, rifle.id);
  store.close();
...
test('v108 and v109 install Shrooms and restrict their mine to Bromo cities', (context) => {
  ...
  assert.deepEqual(availability.map((entry) => entry.city_name), ['Ashfall']);
  assert.equal(store.database.prepare(`
...
test('installs Wood finds and hardware Melds only in Calbuco', (context) => {
  ...
  assert.equal(availability[0].city_name, 'Stormcrag');
  assert.equal(store.database.prepare(`
    SELECT COUNT(*) AS count FROM catalog_items
    WHERE mine_type_id = ? AND rarity >= 5 AND description NOT LIKE '%magic%'
...
test('installs three-line Wisdom haiku and Melds only in Dempo', (context) => {
  ...
  assert.equal(availability[0].city_name, 'Emberdeep');
  assert.equal(store.database.prepare(`
    SELECT COUNT(*) AS count
    FROM catalog_city_mine_types
    JOIN catalog_cities ON catalog_cities.id = catalog_city_mine_types.city_id
...
test('installs Electronic Devices and their Melds only in Ebeko', (context) => {
  ...
  assert.equal(availability[0].city_name, 'Frostmere');
  assert.equal(store.database.prepare(`
    SELECT COUNT(*) AS count
    FROM catalog_city_mine_types
    JOIN catalog_cities ON catalog_cities.id = catalog_city_mine_types.city_id
...
test('installs ominous Relics and their Melds only in Fogo', (context) => {
  ...
  assert.equal(availability[0].city_name, 'Brimstone');
  assert.equal(store.database.prepare(`
    SELECT COUNT(*) AS count
    FROM catalog_city_mine_types
    JOIN catalog_cities ON catalog_cities.id = catalog_city_mine_types.city_id
...
test('normalizes and enforces one city marker per mine type in each region', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-regional-mines-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  const calbuco = store.database.prepare(
    "SELECT id, capital_city_id FROM world_maps WHERE slug = 'calbuco'"
  ).get();
  const otherCity = store.database.prepare(`
    SELECT id FROM catalog_cities WHERE map_id = ? AND id <> ? ORDER BY id LIMIT 1
  `).get(calbuco.id, calbuco.capital_city_id);
  store.database.exec(`
    DROP TRIGGER catalog_city_mine_types_one_per_region_insert;
    DROP TRIGGER catalog_city_mine_types_one_per_region_update;
    DROP TRIGGER catalog_cities_preserve_mine_type_regions;
    DELETE FROM schema_migrations WHERE name = 'regional-mine-type-uniqueness-v1';
  `);
  const nextAvailabilityId = store.database.prepare(
    'SELECT MAX(id) + 1 AS id FROM catalog_city_mine_types'
  ).get().id;
  store.database.prepare(`
    INSERT INTO catalog_city_mine_types (id, city_id, mine_type_id) VALUES (?, ?, ?)
  `).run(nextAvailabilityId, otherCity.id, WOOD_CATALOG.mineType.id);
  store.close();

  store = new SqliteStore(databaseFile);
  const availability = store.database.prepare(`
    SELECT catalog_city_mine_types.city_id
    FROM catalog_city_mine_types
    JOIN catalog_cities ON catalog_cities.id = catalog_city_mine_types.city_id
    WHERE catalog_cities.map_id = ? AND catalog_city_mine_types.mine_type_id = ?
  `).all(calbuco.id, WOOD_CATALOG.mineType.id);
  assert.deepEqual(availability.map((entry) => entry.city_id), [calbuco.capital_city_id]);
  assert.throws(() => store.database.prepare(`
    INSERT INTO catalog_city_mine_types (id, city_id, mine_type_id) VALUES (?, ?, ?)
  `).run(nextAvailabilityId + 1, otherCity.id, WOOD_CATALOG.mineType.id),
  /only one city per region/);
});
...
test('seeds and reloads the legacy gameplay catalog from SQLite', (context) => {
  ...
  assert.equal(restored.gadgetItems.length, 42);
  assert.equal(restored.factoryActions.length, 22);
  ...
  assert.equal(restored.mods.length, 73);
  ...
  assert.equal(restored.stones.length, 82);
});
...
test('awards each original stone once and persists achievement progress', (context) => {
  ...
  assert.equal(progress.next.length, catalog.stones.length - 1);
...
test('runs the four replacement gadget automations against real city systems', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  ...
  const occupiedMineTypeIds = new Set(
    [shipType.itemId, cannon.itemId, ammunition.itemId, catalog.settings.ore_item_id]
      .map((itemId) => catalog.byId.get(itemId)?.mineTypeId)
  );
  const listedItemsByType = Map.groupBy(catalog.items.filter((item) => item.marketableId
    && !item.damaged && !occupiedMineTypeIds.has(item.mineTypeId)
    && !catalog.gadgetItemByItemId.has(item.id)), (item) => item.mineTypeId);
  const listedItems = [...listedItemsByType.values()].find((items) => items.length >= 2)?.slice(0, 2);
  const listedItem = listedItems?.[0];
  const meldIngredient = catalog.items.find((item) => item.marketableId
    && item.mineTypeId !== listedItem?.mineTypeId && !catalog.gadgetItemByItemId.has(item.id));
  assert.ok(shipType && cannon && ammunition && listedItems && meldIngredient);

  const player = createPlayer('Automator', '', 'hash', catalog, 1000, () => 0.5);
  ...
  player.inventory[listedItems[0].id] = 3;
  player.inventory[listedItems[1].id] = 2;
  player.inventory[meldIngredient.id] = 1;
  player.inventory[catalog.settings.ore_item_id] = 100;
  const saved = store.addPlayer(player);
  const vehicleId = store.activateVehicle(saved.id, shipType.itemId);
  store.attachShipCannon(saved.id, vehicleId, cannon.id);
  for (const itemId of [261, 263, 273, 275]) store.activateGadget(saved.id, itemId, 2000);

  for (const invalidQuantity of [0, 1.5, 1001]) {
    assert.throws(() => store.configureGadgetAutomation(saved.id, 'autoloader', {
      vehicleId, ammoType: ammunition.type, quantity: invalidQuantity, intervalMinutes: 5
    }, 3000), /whole number of ammunition crates/u);
  }

  const factoryAction = catalog.factoryActions.find((action) => action.actionKind === 'item');
  const factoryId = Number(store.database.prepare(`
    ...
    VALUES (?, ?, ?, 1, NULL, NULL, 0, 2000, NULL, 2000)
  `).run(saved.id, saved.id, saved.cityId).lastInsertRowid);
  store.database.prepare(`
    INSERT INTO catalog_melds (id, name, mine_type_id, modified, rarity, is_public)
    VALUES (0, 'Automation Test Meld', 1, NULL, 6, 1)
  `).run();
  store.database.prepare(`
    INSERT INTO catalog_meld_requirements (id, meld_id, item_id, quantity)
    VALUES (9999, 0, ?, 1)
  `).run(meldIngredient.id);

  store.configureGadgetAutomation(saved.id, 'autoloader', {
    vehicleId, ammoType: ammunition.type, quantity: 1, intervalMinutes: 5
  }, 3000);
  store.configureGadgetAutomation(saved.id, 'autolister', {
    cityId: saved.cityId, mineTypeId: listedItem.mineTypeId,
    markupPercent: 25, intervalMinutes: 5
  }, 3000);
  store.configureGadgetAutomation(saved.id, 'automaker', {
    factoryId, actionId: factoryAction.id, intervalMinutes: 5
  }, 3000);
  store.configureGadgetAutomation(saved.id, 'automelder', {
    cityId: saved.cityId, meldId: 0, intervalMinutes: 5
  }, 3000);

  const result = store.settleGadgetAutomations(3000);
  assert.equal(result.automationsProcessed, 4);
  assert.equal(result.actions, 4);
  assert.equal(result.disabled, 1, JSON.stringify(store.database.prepare(`
    SELECT catalog_gadgets.behavior_key, gadget_automations.last_status
    FROM gadget_automations JOIN catalog_gadgets
      ON catalog_gadgets.id = gadget_automations.gadget_id
    WHERE gadget_automations.player_id = ? ORDER BY catalog_gadgets.id
  `).all(saved.id)));
  const ammoField = catalog.settings.ammunition_rules[ammunition.type].storageField;
  assert.equal(store.database.prepare(
    `SELECT ${ammoField} AS shots FROM player_ship_state WHERE vehicle_id = ?`
  ).get(vehicleId).shots, Number(catalog.settings.shots_per_crate));
  assert.equal(store.database.prepare(`
    SELECT quantity FROM inventory WHERE player_id = ? AND city_id = ? AND item_id = ?
  `).get(saved.id, saved.cityId, ammunition.itemId).quantity, 1);
  const listings = store.database.prepare(`
    SELECT quantity, price_units FROM market_orders
    WHERE player_id = ? AND city_id = ? AND item_id = ? AND side = 'sell'
  `);
  for (const [index, item] of listedItems.entries()) {
    const listing = listings.get(saved.id, saved.cityId, item.id);
    assert.equal(listing.quantity, index === 0 ? 3 : 2);
    const localMinimum = Math.round(
      store.marketForItem(item.id, saved.cityId).minimumPrice * 10000
    );
    assert.equal(listing.price_units,
      roundListingMinimumUpUnits(Math.ceil(localMinimum * 1.25)));
  }
  assert.equal(store.database.prepare(
    'SELECT factory_action_id FROM factories WHERE id = ?'
  ).get(factoryId).factory_action_id, factoryAction.id);
  assert.ok(store.database.prepare(
    'SELECT 1 FROM player_melds WHERE player_id = ? AND meld_id = 0'
  ).get(saved.id));
  assert.equal(store.database.prepare(`
    SELECT enabled FROM gadget_automations
    JOIN catalog_gadgets ON catalog_gadgets.id = gadget_automations.gadget_id
    WHERE player_id = ? AND catalog_gadgets.behavior_key = 'automelder'
  `).get(saved.id).enabled, 0);
  assert.ok(store.stonesForPlayer(saved.id).earned.some(
    (stone) => stone.behaviorKey === 'Automated'
  ));
  assert.equal(store.settleGadgetAutomations(3001).automationsProcessed, 0);
});

test('Autolister offers and lists protected factory-made products', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  store.ensureWorldMaps(1000);
  const autolister = catalog.gadgetByName.get('autolister');
  const factoryAction = catalog.factoryActions.find((action) => {
  ...
  const output = catalog.byId.get(factoryAction?.outputItemId);
  assert.ok(autolister && factoryAction && output);

  const draft = createPlayer('Factory Stock Lister', '', 'hash', catalog, 1000, () => 0.5);
  draft.inventory = { [output.id]: 2 };
  draft.inventoryByCity = { [draft.cityId]: draft.inventory };
  const player = store.addPlayer(draft);
  store.database.prepare(`
    INSERT INTO protected_inventory (player_id, city_id, item_id, quantity)
    VALUES (?, ?, ?, 2)
  `).run(player.id, player.cityId, output.id);
  store.database.prepare(`
    INSERT INTO player_gadgets (player_id, gadget_id, expires_at)
    VALUES (?, ?, 9999999999)
  `).run(player.id, autolister.id);

  const report = store.gadgetAutomationReport(player.id, 'autolister', 2000);
  const stockType = report.stockTypes.find((entry) =>
    Number(entry.id) === Number(output.mineTypeId)
      && Number(entry.cityId) === Number(player.cityId));
  assert.deepEqual({ quantity: Number(stockType?.quantity), itemCount: Number(stockType?.itemCount) },
    { quantity: 2, itemCount: 1 });
  assert.equal(store.database.prepare(
    'SELECT marketable_id FROM catalog_items WHERE id = ?'
  ).get(output.id).marketable_id, null,
  'the factory-product rule, not a legacy marketable ID, makes this stock eligible');

  store.configureGadgetAutomation(player.id, 'autolister', {
    cityId: player.cityId,
    mineTypeId: output.mineTypeId,
    markupPercent: 10,
    intervalMinutes: 5
  }, 2000);
  assert.deepEqual(store.settleGadgetAutomations(2000), {
    automationsProcessed: 1, actions: 1, disabled: 0
  });
  const listing = store.database.prepare(`
    SELECT quantity, price_units FROM market_orders
    WHERE player_id = ? AND city_id = ? AND item_id = ? AND side = 'sell'
  `).get(player.id, player.cityId, output.id);
  assert.equal(listing.quantity, 2);
  const localMinimum = Math.round(
    store.marketForItem(output.id, player.cityId).minimumPrice * 10000
  );
  assert.equal(listing.price_units,
    roundListingMinimumUpUnits(Math.ceil(localMinimum * 1.1)));
  assert.equal(store.database.prepare(`
    SELECT quantity FROM protected_inventory
    WHERE player_id = ? AND city_id = ? AND item_id = ?
  `).get(player.id, player.cityId, output.id).quantity, 2,
  'listing factory output keeps its no-recycling protection while it remains owned');
});

test('keeps Automaker jobs recurring and lets miners re-enable a paused schedule', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  store.ensureWorldMaps(1000);
  const automaker = catalog.gadgetByName.get('automaker');
  const action = catalog.factoryActions.find((candidate) => candidate.actionKind === 'item');
  assert.ok(automaker && action);
  const draft = createPlayer('Recurring Automaker', '', 'hash', catalog, 1000, () => 0.5);
  draft.inventory[catalog.settings.ore_item_id] = 1000;
  const player = store.addPlayer(draft);
  store.database.prepare(`
    INSERT INTO player_gadgets (player_id, gadget_id, expires_at) VALUES (?, ?, 9999999999)
  `).run(player.id, automaker.id);
  const factoryId = Number(store.database.prepare(`
    INSERT INTO factories
      (owner_id, operator_id, city_id, built, factory_action_id, item_id,
       components_done, last_event_at, completion_at, created_at)
    VALUES (?, ?, ?, 1, NULL, NULL, 0, 1000, NULL, 1000)
  ...
  store.database.prepare(`
    ...
    VALUES (?, ?, ?, 1, 'Fast test bot', 1000000000, 9999999999, 1000)
  `).run(player.id, factoryId, player.cityId);

  store.configureGadgetAutomation(player.id, 'automaker', {
    factoryId, actionId: action.id, intervalMinutes: 5
  }, 3000);
  assert.deepEqual(store.settleGadgetAutomations(3000), {
    automationsProcessed: 1, actions: 1, disabled: 0
  });
  const outputBefore = Number(store.database.prepare(`
    SELECT quantity FROM inventory WHERE player_id = ? AND city_id = ? AND item_id = ?
  `).get(player.id, player.cityId, action.outputItemId)?.quantity ?? 0);
  assert.deepEqual(store.settleGadgetAutomations(303000), {
    automationsProcessed: 1, actions: 1, disabled: 0
  });
  const outputAfter = Number(store.database.prepare(`
    SELECT quantity FROM inventory WHERE player_id = ? AND city_id = ? AND item_id = ?
  `).get(player.id, player.cityId, action.outputItemId)?.quantity ?? 0);
  assert.equal(outputAfter, outputBefore + action.outputQuantity);
  let report = store.gadgetAutomationReport(player.id, 'automaker', 303000);
  assert.equal(report.enabled, true);
  assert.equal(report.tasks.length, 1);
  assert.equal(store.database.prepare(
    'SELECT factory_action_id FROM factories WHERE id = ?'
  ).get(factoryId).factory_action_id, action.id);

  store.database.prepare('UPDATE factories SET built = 0 WHERE id = ?').run(factoryId);
  assert.deepEqual(store.settleGadgetAutomations(603000), {
    automationsProcessed: 1, actions: 1, disabled: 1
  });
  report = store.gadgetAutomationReport(player.id, 'automaker', 603000);
  assert.equal(report.enabled, false);
  assert.equal(report.tasks.length, 1, 'pausing Automaker must retain its recurring job');
  store.database.prepare('UPDATE factories SET built = 1 WHERE id = ?').run(factoryId);
  store.enableGadgetAutomation(player.id, 'automaker', 603001);
  report = store.gadgetAutomationReport(player.id, 'automaker', 603001);
  assert.equal(report.enabled, true);
  assert.equal(report.nextRunAt, 603001);
  assert.equal(report.tasks.length, 1);
});

test('runs ten automation tasks in round-robin order and rejects an eleventh', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  store.ensureWorldMaps(1000);
  const items = [];
  const selectedMineTypeIds = new Set();
  for (const item of catalog.items.filter((candidate) => candidate.marketableId
    && !candidate.damaged)) {
    if (selectedMineTypeIds.has(item.mineTypeId)) continue;
    selectedMineTypeIds.add(item.mineTypeId);
    items.push(item);
    if (items.length === 11) break;
  }
  assert.equal(items.length, 11);
  const draft = createPlayer('Ten Task Automator', '', 'hash', catalog, 1000, () => 0.5);
  draft.inventory = {};
  draft.inventoryByCity[draft.cityId] = draft.inventory;
  for (const item of items) draft.inventory[item.id] = 1;
  const player = store.addPlayer(draft);
  const autolister = catalog.gadgetByName.get('autolister');
  store.database.prepare(`
    INSERT INTO player_gadgets (player_id, gadget_id, expires_at) VALUES (?, ?, 9999999999)
  ...
  for (const item of items.slice(0, 10)) {
    store.configureGadgetAutomation(player.id, 'autolister', {
      cityId: player.cityId, mineTypeId: item.mineTypeId, markupPercent: 10,
      intervalMinutes: 5
    }, 3000);
  }
  assert.throws(() => store.configureGadgetAutomation(player.id, 'autolister', {
    cityId: player.cityId, mineTypeId: items[10].mineTypeId, markupPercent: 10,
    intervalMinutes: 5
  }, 3000), /maximum of 10 tasks/u);
  let report = store.gadgetAutomationReport(player.id, 'autolister', 3000);
  assert.equal(report.taskLimit, 10);
  assert.equal(report.tasks.length, 10);
  assert.equal(report.configuration.cursor, 0);

  for (let index = 0; index < 10; index += 1) {
    const result = store.settleGadgetAutomations(3000 + index * 5 * 60 * 1000);
    assert.deepEqual(result, { automationsProcessed: 1, actions: 1, disabled: 0 });
    const listedIds = store.database.prepare(`
      SELECT item_id FROM market_orders WHERE player_id = ? AND side = 'sell'
      ORDER BY id
    `).all(player.id).map((row) => row.item_id);
    assert.deepEqual(listedIds, items.slice(0, index + 1).map((item) => item.id));
  }
  report = store.gadgetAutomationReport(player.id, 'autolister', 3000000);
  assert.equal(report.configuration.cursor, 0);
  assert.equal(report.enabled, true);
});

test('builds a read-only operations snapshot without travelling vehicles', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  store.ensureWorldMaps(1000);
  const liveCatalog = store.loadCatalog();
  const landType = liveCatalog.settings.route_type_ids.land;
  ...
  const vehicleType = liveCatalog.vehicles.find((candidate) =>
    candidate.routeType === landType && candidate.capacity > 0);
  const stock = liveCatalog.items.find((candidate) => candidate.marketableId
    && Number.isSafeInteger(candidate.mineTypeId) && !candidate.damaged);
  const autolister = liveCatalog.gadgetByName.get('autolister');
  assert.ok(route && vehicleType && stock && autolister);

  const draft = createPlayer(
    'Map Operations', '', 'hash', liveCatalog, 1000, () => 0.5
  );
  ...
  const eventsBefore = store.database.prepare(
    'SELECT COUNT(*) AS count FROM live_update_events'
  ).get().count;
  const snapshot = store.mapOperationsSnapshot(player.id, 2500);

  assert.equal(snapshot.generatedAt, 2500);
  assert.equal(Object.hasOwn(snapshot, 'inTransit'), false);
  assert.equal(snapshot.shuttles.length, 1);
  ...
  assert.deepEqual(snapshot.automations[0].tasks, [{
    cityId: route.city1Id, mineTypeId: stock.mineTypeId,
    markupPercent: 25, index: 0
  }]);
  assert.equal(store.database.prepare(
    'SELECT COUNT(*) AS count FROM live_update_events'
  ).get().count, eventsBefore, 'reading the snapshot must not publish live changes');
  assert.equal(store.database.prepare(
    "SELECT COUNT(*) AS count FROM player_vehicles WHERE player_id = ? AND status = 'traveling'"
  ).get(player.id).count, 1, 'reading the snapshot must not settle journeys');
});
...
test('buys the original ten-day battery extension only while charged', (context) => {
  ...
  assert.ok(store.stonesForPlayer(player.id).earned.some(
    (stone) => stone.behaviorKey === 'Extended'
  ));
...
test('does not manufacture historical specialisation tenure after state is reset', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const startedAt = 1000;
  const player = store.addPlayer(
    createPlayer('Reset Tenure', '', 'hash', catalog, startedAt, () => 0.5)
  );
  store.database.prepare('DELETE FROM specialisation_tenure WHERE player_id = ?')
    .run(player.id);

  const reopenedAt = startedAt + 100 * Number(catalog.settings.day_ms);
  const restored = store.playerById(player.id, reopenedAt);
  const tenure = restored.specialisationTenure.find(
    (entry) => entry.specialisationId === restored.profession
  );
  assert.equal(tenure.activeMs, 0);
  assert.equal(store.database.prepare(`
    SELECT settled_at FROM specialisation_tenure
    WHERE player_id = ? AND specialisation_id = ?
  `).get(player.id, restored.profession).settled_at, reopenedAt);
});
...
test('purchases prerequisite-gated original bot parts with gold', (context) => {
  ...
  const first = store.buyBotPart(player.id, 1, 2000);
  assert.equal(first.name, 'chest');
  assert.equal(first.botPartNumber, 1);
  assert.equal(first.botPartTotal, 13);
  const second = store.buyBotPart(player.id, 3, 3000);
  assert.equal(second.name, 'LUarm');
  assert.equal(second.botPartNumber, 2);
  assert.equal(second.botPartTotal, 13);
  const restored = store.playerById(player.id);
...
test('persists a proportionally rebased mine schedule after a rate upgrade', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const player = store.addPlayer(
    createPlayer('Scheduled Builder', '', 'hash', catalog, 1000, () => 0.5)
  );
  const now = 10_000;
  const oldInterval = player.mines[0].cycleIntervalMs;
  store.database.prepare(`
    UPDATE mines SET next_find_at = ?, cycle_interval_ms = ?
    WHERE player_id = ? AND id = ?
  `).run(now + Math.round(oldInterval / 2), oldInterval, player.id, player.mines[0].id);
  store.database.prepare('UPDATE players SET gold_units = 10000 WHERE id = ?').run(player.id);

  store.buyBotPart(player.id, 1, now);

  const restored = store.playerById(player.id, now);
  const mine = restored.mines[0];
  const fasterInterval = mineIntervalMs(catalog, mine, restored, now);
...
test('clears the Assembled Stone when the starter miner bot is complete', (context) => {
  ...
  for (const [index, part] of catalog.botParts.entries()) {
    ...
    assert.equal(result.botPartNumber, index + 1);
    assert.equal(result.botPartTotal, catalog.botParts.length);
...
test('settles automatic mine gold exactly at specialisation changes', (context) => {
  ...
  assert.equal(store.playerById(savedEntering.id, 1000 + 2 * hour).gold, 6.2444,
    'time before becoming a Bum is not backdated and the new Stone boosts mining');
...
test('runs meld-gated factory construction and damaged-item repair with contracted workers', (context) => {
  ...
  assert.ok(store.stonesForPlayer(maker.id).earned.some((stone) => stone.name === 'Employed'));
  assert.ok(store.stonesForPlayer(maker.id).earned.some((stone) => stone.name === 'Restored'));
...
test('manufactures Bolt boxes and breaks them down into protected individual Bolts', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const maker = store.addPlayer(createPlayer(
    'Bolt Boxer', '', 'hash', catalog, 1000, () => 0.5
  ));
  const box = BOLT_BOX_CATALOG.items[0];
  const action = BOLT_BOX_CATALOG.factoryActions[0];
  const before = store.playerById(maker.id);
  const boltsBefore = before.inventory[BOLT_BOX_CATALOG.boltItemId] ?? 0;
  store.database.prepare(`
    INSERT INTO factories
      (owner_id, operator_id, city_id, built, factory_action_id, item_id,
       components_done, last_event_at, completion_at, created_at)
    VALUES (?, ?, ?, 1, ?, NULL, ?, 1000, NULL, 1000)
  `).run(maker.id, maker.id, maker.cityId, action.id, action.components);

  const completed = store.factoriesForPlayer(maker.id, 2000).completions[0];
  assert.equal(completed.actionName, action.name);
  const boxed = store.playerById(maker.id);
  assert.equal(boxed.inventory[box.id], 1);
  assert.equal(boxed.protectedInventoryByCity[maker.cityId][box.id], 1);

  const opened = store.breakDownBoltBoxes(maker.id, 1);
  assert.deepEqual(opened, {
    boxItemId: box.id,
    boxName: box.name,
    boltItemId: BOLT_BOX_CATALOG.boltItemId,
    boltName: catalog.byId.get(BOLT_BOX_CATALOG.boltItemId).name,
    boxes: 1,
    bolts: 20,
    boltsPerBox: 20
  });
  const unpacked = store.playerById(maker.id);
  assert.equal(unpacked.inventory[box.id], undefined);
  assert.equal(unpacked.inventory[BOLT_BOX_CATALOG.boltItemId], boltsBefore + 20);
  assert.equal(unpacked.protectedInventoryByCity[maker.cityId][BOLT_BOX_CATALOG.boltItemId], 20);
  assert.ok(store.stonesForPlayer(maker.id).earned.some(
    (stone) => stone.behaviorKey === 'Unboxed'
  ));
  assert.throws(() => store.breakDownBoltBoxes(maker.id, 1), /do not own enough/u);
});

test('opens a Safe Travel Kit into local supplies and kept fitting instructions', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const playerState = createPlayer(
    'Safe Traveller', '', 'hash', catalog, 1000, () => 0.5
  );
  playerState.inventory[SAFE_TRAVEL_KIT_CATALOG.itemId] = 1;
  const player = store.addPlayer(playerState);
  const before = store.playerById(player.id);
  const beforeCounts = new Map(SAFE_TRAVEL_KIT_CATALOG.contents.map((content) => [
    content.itemId, Number(before.inventory[content.itemId] ?? 0)
  ]));

  const result = store.openSafeTravelKit(player.id, 2000);
  assert.equal(result.kitItemId, SAFE_TRAVEL_KIT_CATALOG.itemId);
  assert.equal(result.cityId, player.cityId);
  assert.equal(result.boltsRequired, 0);
  assert.deepEqual(result.contents, SAFE_TRAVEL_KIT_CATALOG.contents);
  const opened = store.playerById(player.id);
  ...
  for (const content of SAFE_TRAVEL_KIT_CATALOG.contents) {
    assert.equal(opened.inventory[content.itemId],
      beforeCounts.get(content.itemId) + content.quantity);
  }

  const message = store.messageForPlayer(player.id, result.messageId);
  assert.equal(message.senderName, 'The Council');
  assert.equal(message.subject, 'Safe Travel Kit fitting instructions');
  assert.equal(message.kept, true);
  assert.equal(message.details.event, 'safe-travel-kit-opened');
  assert.deepEqual(message.details.contents, SAFE_TRAVEL_KIT_CATALOG.contents);
  assert.match(message.body, /5 x Bolt/u);
  assert.match(message.body, /Yellow Tin Door Panels \(\+8 armour, -1 cargo capacity\)/u);
  assert.match(message.body, /Yellow Tin Quick Shift \(\+2 dodge, -1 cargo capacity\)/u);
  assert.match(message.body, /Do not merely carry them as cargo/u);
  assert.match(message.body, /These Yellow mods cost no Bolts to fit/u);
  assert.match(message.body, /select the Peaceful order/u);
  assert.match(message.body, /can still attack it/u);
  assert.deepEqual(message.details.actions, [
  ...
  assert.throws(() => store.openSafeTravelKit(player.id, 3000), /do not own/u);
});

test('installs Safe Travel Kits and grants one to every existing miner exactly once', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-safe-travel-kit-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const existing = store.addPlayer(createPlayer(
    'Retrospective Traveller', '', 'hash', catalog, 1000, () => 0.5
  ));
  const starterPack = JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'starter_welcome_pack'
  `).get().value_json);
  delete starterPack.safeTravelKitItemId;
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = ? WHERE key = 'starter_welcome_pack'
  `).run(JSON.stringify(starterPack));
  store.database.prepare(`
    DELETE FROM catalog_settings
    WHERE key IN ('safe_travel_kit_item_id', 'safe_travel_kit_contents')
  `).run();
  store.database.prepare('DELETE FROM catalog_items WHERE id = ?')
    .run(SAFE_TRAVEL_KIT_CATALOG.itemId);
  store.database.prepare(
    "DELETE FROM schema_migrations WHERE name = 'safe-travel-kit-v1'"
  ).run();
  store.close();
  ...
  assert.equal(store.loadCatalog().byId.get(SAFE_TRAVEL_KIT_CATALOG.itemId).name,
    'Safe Travel Kit');
  assert.equal(store.loadCatalog().settings.starter_welcome_pack.safeTravelKitItemId,
    SAFE_TRAVEL_KIT_CATALOG.itemId);
  const [notice] = store.recentMessages(existing.id, 'all', 'Admin');
  assert.equal(notice.subject, 'Safe Travel Kit issued');
  assert.equal(notice.details.event, 'safe-travel-kit-issued');
  assert.match(notice.body, /opening it unpacks them and reveals/u);
  const migration = store.database.prepare(`
    SELECT details_json FROM schema_migrations WHERE name = 'safe-travel-kit-v1'
  `).get();
  assert.deepEqual(JSON.parse(migration.details_json), {
    itemId: SAFE_TRAVEL_KIT_CATALOG.itemId,
    contents: SAFE_TRAVEL_KIT_CATALOG.contents,
    starterPackChanged: true,
    playersGranted: 1
  });
  ...
  store = new SqliteStore(databaseFile);
  assert.equal(store.playerById(existing.id).inventory[SAFE_TRAVEL_KIT_CATALOG.itemId], 1);
  assert.equal(store.recentMessages(existing.id, 'all', 'Admin').length, 1);
});

test('adds the Bolt box catalog to an existing schema without a version bump', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-bolt-box-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  const box = BOLT_BOX_CATALOG.items[0];
  const action = BOLT_BOX_CATALOG.factoryActions[0];
  store.database.prepare('DELETE FROM catalog_factory_actions WHERE id = ?').run(action.id);
  store.database.prepare('DELETE FROM catalog_items WHERE id = ?').run(box.id);
  store.database.prepare(`
    DELETE FROM catalog_settings WHERE key IN ('bolt_box_item_id', 'bolts_per_box')
  `).run();
  store.database.prepare(
    "DELETE FROM schema_migrations WHERE name = 'bolt-box-catalog-v1'"
  ).run();
  store.close();
  ...
  assert.equal(restored.factoryActions.find((entry) => entry.id === action.id).outputItemId,
    box.id);
  assert.equal(restored.settings.bolts_per_box, 20);
  assert.ok(store.database.prepare(
    "SELECT 1 FROM schema_migrations WHERE name = 'bolt-box-catalog-v1'"
  ).get());
});

test('adds the Magnet factory product to an existing schema without a version bump', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-magnet-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const magnet = MAGNET_CATALOG.items[0];
  const action = MAGNET_CATALOG.factoryActions[0];
  store.database.prepare('DELETE FROM catalog_factory_actions WHERE id = ?').run(action.id);
  store.database.prepare('DELETE FROM catalog_mods WHERE id = ?').run(MAGNET_CATALOG.mod.id);
  store.database.prepare('DELETE FROM catalog_items WHERE id = ?').run(magnet.id);
  store.database.prepare(`
    DELETE FROM catalog_settings
    WHERE key IN ('magnet_item_id', 'magnet_intact_salvage_chance', 'magnet_fitting_bolt_cost')
  `).run();
  store.database.prepare(
    "DELETE FROM schema_migrations WHERE name = 'magnet-catalog-v1'"
  ).run();
  store.close();
  ...
  assert.equal(restored.factoryActions.find((entry) => entry.id === action.id).outputItemId,
    magnet.id);
  assert.equal(restored.settings.magnet_item_id, magnet.id);
  assert.equal(restored.settings.magnet_intact_salvage_chance,
    MAGNET_CATALOG.intactSalvageChance);
  assert.equal(restored.settings.magnet_fitting_bolt_cost, 1);
  assert.ok(store.database.prepare(
    "SELECT 1 FROM schema_migrations WHERE name = 'magnet-catalog-v1'"
  ).get());
});

test('upgrades existing Magnets with artwork and the one-Bolt vehicle fitting', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-magnet-fitting-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.database.prepare(`
    UPDATE catalog_items
    SET description = ?, icon = '', icon_source = '', large_image = NULL, has_large_image = 0
    WHERE id = ?
  `).run(legacyDescription, magnet.id);
  store.database.prepare('DELETE FROM catalog_mods WHERE id = ?').run(MAGNET_CATALOG.mod.id);
  store.database.prepare(
    "DELETE FROM catalog_settings WHERE key = 'magnet_fitting_bolt_cost'"
  ).run();
  store.database.prepare(`
    INSERT OR REPLACE INTO schema_migrations (name, applied_at, details_json)
    VALUES ('magnet-catalog-v1', 2000, '{}')
  `).run();
  store.database.prepare(
    "DELETE FROM schema_migrations WHERE name = 'magnet-fitting-v1'"
  ).run();
  store.close();

  store = new SqliteStore(databaseFile);
  const restored = store.loadCatalog();
  const restoredMagnet = restored.byId.get(magnet.id);
  assert.equal(restoredMagnet.icon, '/node/equipment/magnet.svg');
  assert.equal(restoredMagnet.largeImage, '/node/equipment/magnet.svg');
  assert.equal(restoredMagnet.description, magnet.description);
  assert.deepEqual(restored.mods.find((mod) => mod.itemId === magnet.id),
    MAGNET_CATALOG.mod);
  assert.equal(restored.settings.magnet_fitting_bolt_cost, 1);
  const migration = store.database.prepare(`
    SELECT details_json FROM schema_migrations WHERE name = 'magnet-fitting-v1'
  `).get();
  assert.deepEqual(JSON.parse(migration.details_json), {
    itemId: magnet.id,
    modId: MAGNET_CATALOG.mod.id,
    modInserted: 1,
    settingInserted: 1,
    artworkUpdated: 1,
...
test('builds mills from Calbuco onward and turns Wood into vehicle reinforcement', (context) => {
  ...
  const screwItemId = WOOD_CATALOG.screwItemId;
  ...
  stock.run(maker.id, calbuco.capital_city_id, wood.id, 2);
  stock.run(maker.id, calbuco.capital_city_id, screwItemId, 8);
  stock.run(maker.id, calbuco.capital_city_id, landVehicle.itemId, 2);
  ...
  assert.equal(store.startMillReinforcement(
    maker.id, mill.id, vehicleId, wood.id, builtAt).enqueued, false);
  const queued = store.startMillReinforcement(
    maker.id, mill.id, queuedVehicleId, wood.id, builtAt + 1);
  assert.equal(queued.enqueued, true);
  assert.equal(queued.queue.length, 1);
  assert.equal(queued.queue[0].targetVehicleId, queuedVehicleId);
  assert.equal(queued.queue[0].screwsReserved, 4);
  assert.equal(store.playerById(maker.id).inventory[screwItemId], undefined);
  assert.equal(store.playerById(maker.id).inventory[wood.id], undefined);
  assert.equal(store.cancelFactoryQueueJob(
    maker.id, mill.id, queued.queuedJob.id, builtAt + 2), 'Reinforce vehicle');
  assert.equal(store.playerById(maker.id).inventory[screwItemId], 4);
  assert.equal(store.playerById(maker.id).inventory[wood.id], 1);
  store.startMillReinforcement(maker.id, mill.id, queuedVehicleId, wood.id, builtAt + 3);
  assert.equal(store.millsForPlayer(maker.id, builtAt + 3).vehicles.length, 0);
  ...
  const firstCompletion = store.millsForPlayer(maker.id, finishedAt).completions[0];
  assert.equal(firstCompletion.actionName, 'Reinforce vehicle');
  assert.equal(firstCompletion.nextActionName, 'Reinforce vehicle');
  const runningMill = store.millsForPlayer(maker.id, finishedAt).mills[0];
  assert.equal(runningMill.targetVehicleId, queuedVehicleId);
  assert.equal(runningMill.queue.length, 0);
  ...
  const secondFinishedAt = finishedAt
    + (reinforceAction.components / millBot.cph) * 60 * 60 * 1000;
  assert.equal(store.millsForPlayer(maker.id, secondFinishedAt).completions[0].actionName,
    'Reinforce vehicle');
  assert.equal(store.database.prepare(`
    SELECT reinforcement_strength FROM player_vehicles WHERE id = ?
  `).get(queuedVehicleId).reinforcement_strength, 5);
  stock.run(maker.id, calbuco.capital_city_id, wood.id, 1);
  stock.run(maker.id, calbuco.capital_city_id, landVehicle.itemId, 1);
  const vehicleWithoutScrews = store.activateVehicle(
    maker.id, landVehicle.itemId, secondFinishedAt);
  assert.throws(() => store.startMillReinforcement(
    maker.id, mill.id, vehicleWithoutScrews, wood.id, secondFinishedAt),
    /need 4 Wood Screws/);
...
test('hard-caps factory queues at ten jobs', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = '25' WHERE key = 'factory_queue_limit'
  `).run();
  const draft = createPlayer('Capped Queue Maker', '', 'hash', catalog, 1000, () => 0.5);
  draft.inventory[catalog.settings.ore_item_id] = 1000;
  const player = store.addPlayer(draft);
  const factoryId = Number(store.database.prepare(`
    INSERT INTO factories
    ...
    VALUES (?, ?, ?, 1, NULL, NULL, 0, 1000, NULL, NULL, 1000)
  `).run(player.id, player.id, player.cityId).lastInsertRowid);
  const action = catalog.factoryActions.find((candidate) => candidate.name === 'Flower');
  assert.ok(action);
  store.startFactoryAction(player.id, factoryId, action.id, null, 2000);
  for (let index = 0; index < 10; index += 1) {
    store.startFactoryAction(player.id, factoryId, action.id, null, 2001 + index);
  }
  assert.throws(
    () => store.startFactoryAction(player.id, factoryId, action.id, null, 2011),
    /queue is full \(10 jobs\)/u
  );
  const factory = store.factoriesForPlayer(player.id, 2012).factories[0];
  assert.equal(factory.queueLimit, 10);
  assert.equal(factory.queue.length, 10);
});
...
test('raises legacy Ore and manufactured-item floors to cover production inputs', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-production-prices-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const player = store.addPlayer(createPlayer(
    'Legacy Manufacturer', '', 'hash', catalog, 1000, () => 0.5
  ));
  const action = catalog.factoryActions.find((candidate) => candidate.name === 'Search Plane');
  assert.ok(action);
  const legacyRules = structuredClone(catalog.settings.item_value_rules);
  delete legacyRules.oreCrateGold;
  legacyRules.factory = { ore: 0.12, components: 1 / 15000 };
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = ? WHERE key = 'item_value_rules'
  `).run(JSON.stringify(legacyRules));
  store.database.prepare(`
    UPDATE catalog_items SET gold_value_units = 10000 WHERE id IN (?, ?)
  `).run(catalog.settings.ore_item_id, action.outputItemId);
  const orderId = Number(store.database.prepare(`
    INSERT INTO market_orders
      (player_id, city_id, item_id, side, price_units, quantity, created_at)
    VALUES (?, ?, ?, 'sell', 10000, 1, 2000)
  `).run(player.id, player.cityId, action.outputItemId).lastInsertRowid);
  store.close();

  store = new SqliteStore(databaseFile);
  const rules = JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'item_value_rules'
  `).get().value_json);
  assert.equal(rules.oreCrateGold, 10);
  assert.deepEqual(rules.factory, {
    ore: 0.12,
    components: 1 / 15000,
    oreInputValueShare: 1,
    workerCostShare: 1
  });
  assert.equal(store.database.prepare(
    'SELECT gold_value_units FROM catalog_items WHERE id = ?'
  ).get(catalog.settings.ore_item_id).gold_value_units, 10 * 10000);
  const contractHours = catalog.settings.factory_worker_bot_contract_duration_ms / 3600000;
  const laborGoldPerComponent = Math.min(...catalog.settings.factory_worker_bot_tiers.map(
    (tier) => tier.costGold / (tier.cph * contractHours)
  ));
  const expectedOutputFloor = Math.ceil((
    action.ore * 10 + action.components * laborGoldPerComponent
  ) / action.outputQuantity);
  assert.equal(store.database.prepare(
    'SELECT gold_value_units FROM catalog_items WHERE id = ?'
  ).get(action.outputItemId).gold_value_units, expectedOutputFloor * 10000);
  assert.equal(store.database.prepare(
    'SELECT 1 FROM market_orders WHERE id = ?'
  ).get(orderId), undefined);
  assert.ok(store.database.prepare(`
    SELECT 1 FROM schema_migrations WHERE name = 'manufacturing-minimum-prices-v1'
  `).get());
});
...
test('recycles local inventory into Ore scraps and recommends low-value things when over capacity', (context) => {
  ...
  assert.ok(store.stonesForPlayer(player.id).earned.some(
    (stone) => stone.behaviorKey === 'Recycled'
  ));
  assert.ok(store.stonesForPlayer(player.id).earned.some(
    (stone) => stone.behaviorKey === 'Refined'
  ));
...
test('coalesces Oil Field views without writes and forces settlement before actions', (context) => {
  ...
  const changesBeforeDeferredSettlement = store.database.prepare(
    'SELECT total_changes() AS count'
  ).get().count;
  const deferredSettlement = store.settleOilField(2000, { minimumIntervalMs: 60000 });
  assert.equal(deferredSettlement.settled, false);
  assert.equal(deferredSettlement.settledAt, 1999);
  assert.equal(store.database.prepare('SELECT total_changes() AS count').get().count,
    changesBeforeDeferredSettlement,
    'background Oil Field throttling must not write before its persistence interval');
...
test('blocks creature interception departures in snow without creating a pursuit', (context) => {
  ...
  assert.throws(
    () => store.sendVehicle(saved.id, vehicleId, route.id, departedAt + 1),
    /cannot depart from .* while it is snowing/i
  );
  assert.equal(store.database.prepare(
...
test('a scanned shipwreck can rise as a Ghost Ship at its loss location', (context) => {
  ...
  const damagedShip = catalog.items.find((item) => item.repairedItemId === shipType?.itemId);
  ...
  assert.ok(shipType && damagedShip && cannon && cargo);
  ...
  assert.deepEqual(wrecks.map((wreck) => wreck.item_id).sort((a, b) => a - b),
    [damagedShip.id, cannon.itemId, cargo.id].sort((a, b) => a - b));
  ...
  assert.deepEqual(store.database.prepare(`
    SELECT cannon_id, portal FROM player_ship_cannons
    WHERE vehicle_id = ? ORDER BY portal
  `).all(ghost.vehicle_id).map((entry) => ({ ...entry })), [
    { cannon_id: cannon.id, portal: 1 }
  ], 'a Ghost Ship inherits the wreck cannon instead of receiving invented top-tier guns');
  const ghostShipState = store.database.prepare(`
    SELECT hull, max_hull, massives, chain_shots, grape_shots
    FROM player_ship_state WHERE vehicle_id = ?
  `).get(ghost.vehicle_id);
  const expectedSpectralHull = Math.round(shipType.ship.hull
    * (1 + JSON.parse(store.database.prepare(`
      SELECT value_json FROM catalog_settings WHERE key = 'ghost_combat_bonus'
    `).get().value_json)));
  assert.deepEqual({ ...ghostShipState }, {
    hull: expectedSpectralHull, max_hull: expectedSpectralHull,
    massives: 0, chain_shots: 0, grape_shots: 0
  }, 'a Ghost Ship receives only its spectral hull bonus and the wreck ammunition');
...
test('schedules natural creatures independently from variable weather periods', (context) => {
  ...
  assert.ok(clock.next_roll_at - dueAt <= 10 * 60 * 1000);
...
test('persists natural roll timing and collapses downtime to one current roll', (context) => {
  ...
  assert.ok(advanced.next_roll_at - now <= 10 * 60 * 1000);
  store.close();
...
test('migrates Dwarf and creature rolls to the frequent scheduling windows', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-frequent-rolls-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const oldSettings = {
    dwarf_find_max_delay_ms: 420000,
    dwarf_stowaway_interval_ms: 86400000,
  ...
  store.database.prepare(`
    UPDATE dwarf_state SET next_find_at = ?, next_stowaway_at = ? WHERE id = 1
...
test('moves creatures and resolves vehicle attacks only at a physical interception', (context) => {
  ...
  const pursuit = launchVehicleAtCreature(store, saved.id, creatureId, vehicleId, 2000);
...
test('a Common Land Whale awards Common treasure only', (context) => {
  ...
  store.setVehicleStance(saved.id, vehicleId, 'peaceful', false, true);
  const mapId = store.database.prepare(
    'SELECT map_id FROM catalog_cities WHERE id = ?'
  ).get(saved.cityId).map_id;
  const creatureId = Number(store.database.prepare(`
  ...
  const pursuit = launchVehicleAtCreature(store, saved.id, creatureId, vehicleId, 2000);
  assert.equal(store.vehicleDetails(saved.id, vehicleId, 2001).pveEnabled, true,
    'the saved PvE stance launches the hunt independently of a Peaceful PvP order');
...
test('a PvE stance launches a living-creature hunt from the correct route', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  store.ensureWorldMaps(1000);
  ...
  assert.throws(
    () => store.attackWorldCreature(saved.id, creatureId, vehicleId, awakenedAt),
    /Direct hunts have ended/u
  );
  ...
  const huntingJourney = store.sendVehicle(saved.id, vehicleId, route.id, awakenedAt);
  assert.equal(huntingJourney.huntsLaunched, 1);
  assert.equal(huntingJourney.creatureHuntsLaunched, 1);
  assert.equal(huntingJourney.ghostHuntsLaunched, 0);
  const pursuit = store.database.prepare(`
    SELECT * FROM world_creature_pursuits
    WHERE creature_id = ? AND vehicle_id = ? AND status = 'pursuing'
  `).get(creatureId, vehicleId);
  ...
  assert.equal(report.details.initiator, 'hunter');
  assert.equal(report.details.counterDamage, 1);
  assert.doesNotMatch(report.body, /ambushed your/i);
  assert.match(report.body, /took 1 damage/i);
  assert.equal(store.database.prepare(`
    SELECT COUNT(*) AS count FROM world_creature_pursuits
    WHERE creature_id = ? AND vehicle_id = ?
  `).get(creatureId, vehicleId).count, 1, 'settlement must not plan a duplicate hunt');
});
...
test('a scanned creature wreck can rise as a Wraith Rider', (context) => {
  ...
  assert.ok(route && vehicleType);
  store.database.prepare(
    'UPDATE catalog_lands SET armor = 1 WHERE vehicle_id = ?'
  ).run(vehicleType.id);
  const player = createPlayer('Creature Wreck', '', 'hash', catalog, 1000, () => 0.5);
  player.inventory[vehicleType.itemId] = 1;
  const saved = store.addPlayer(player);
  const vehicleId = store.activateVehicle(saved.id, vehicleType.itemId);
  ...
  const creatureDestinationId = originCityId;
  const location = creatureDestinationId === route.city1Id
    ? route.length * 0.6 : route.length * 0.4;
  const speed = catalog.settings.world_creature_speed_kph.elephant_herd
    * catalog.settings.world_creature_tier_speed_multipliers[1];
  const distanceRemaining = creatureDestinationId === route.city1Id
    ? location : route.length - location;
  const awakenedAt = 2000;
  const arrivesAt = awakenedAt + Math.ceil(distanceRemaining / speed * 60 * 60 * 1000);
  const mapId = store.database.prepare(
    'SELECT map_id FROM catalog_cities WHERE id = ?'
  ).get(creatureDestinationId).map_id;
  const creatureId = Number(store.database.prepare(`
    INSERT INTO world_creatures
      (creature_type, rarity, map_id, route_id, location, spawn_location,
       destination_city_id, hp, max_hp, awakened_at, moved_at, arrives_at, speed)
    VALUES ('elephant_herd', 1, ?, ?, ?, ?, ?, 100000, 100000, ?, ?, ?, ?)
  `).run(mapId, route.id, location, location, creatureDestinationId,
    awakenedAt, awakenedAt, arrivesAt, speed).lastInsertRowid);
  store.sendVehicle(saved.id, vehicleId, route.id, awakenedAt, {
    travelOrder: 'peaceful'
  });
  const dwarfTier = catalog.dwarfTiers.find((tier) =>
    tier.rarity === catalog.byId.get(vehicleType.itemId).rarity);
  const stowawayId = Number(store.database.prepare(`
    ...
    VALUES (?, ?, ?, ?, 'aboard', 1500, ?)
  `).run(dwarfTier.rarity, dwarfTier.itemId, vehicleId, saved.id,
    awakenedAt).lastInsertRowid);
  const pursuit = store.database.prepare(`
    SELECT * FROM world_creature_pursuits
    WHERE creature_id = ? AND vehicle_id = ? AND status = 'pursuing'
  `).get(creatureId, vehicleId);
  ...
  assert.ok(Math.abs(ghost.segment_start_location - pursuit.encounter_location) < 1e-9);
  const report = store.recentMessages(saved.id, 'all', 'Vehicle')
    .find((message) => message.details.creatureId === creatureId);
  ...
  `).get(stowawayId).status, 'escaped');
  assert.match(report.body, /Dwarf stowaway escaped from the wreck/i);
...
test('defeated land and sea event creatures can rise as restless dead once', (context) => {
  const stores = [];
  context.after(() => stores.forEach((store) => store.close()));
  const cases = [
    { type: 'elephant_herd', behavior: 'land', ghostKind: 'rider' },
    { type: 'orca_pod', behavior: 'sea', ghostKind: 'ship' }
  ];
  for (const [index, scenario] of cases.entries()) {
    ...
    for (const [key, value] of [
      ...
      UPDATE catalog_settings SET value_json = ? WHERE key = ?
    `).run(JSON.stringify(value), key);
    const routeType = catalog.settings.route_type_ids[scenario.behavior];
    const route = catalog.routes.find((entry) => entry.open && entry.type === routeType
      && entry.city1Id !== entry.city2Id && [entry.city1Id, entry.city2Id].includes(1));
    const vehicleType = catalog.vehicles.filter((entry) => entry.routeType === routeType
      && catalog.byId.get(entry.itemId)?.rarity === 1)
      .sort((first, second) => second.speed - first.speed)[0];
    assert.ok(route && vehicleType);
    const player = createPlayer(
      `Restless Creature Hunter ${index}`, '', 'hash', catalog, 1000, () => 0.5
    );
    player.inventory[vehicleType.itemId] = 1;
    const saved = store.addPlayer(player);
    const vehicleId = store.activateVehicle(saved.id, vehicleType.itemId);
    ...
    const creatureId = Number(store.database.prepare(`
      INSERT INTO world_creatures
        (creature_type, rarity, map_id, route_id, location, destination_city_id,
         hp, max_hp, awakened_at, moved_at, speed)
      VALUES (?, 1, ?, ?, ?, ?, 1, 1, ?, ?, ?)
    `).run(scenario.type, mapId, route.id, route.length / 2, saved.cityId,
      eventAt, eventAt,
      catalog.settings.world_creature_speed_kph[scenario.type]
        * catalog.settings.world_creature_tier_speed_multipliers[1]).lastInsertRowid);

    const pursuit = launchVehicleAtCreature(
      store, saved.id, creatureId, vehicleId, eventAt
    );
    const settlement = store.settleWorldEvents(pursuit.encounterAt);
    assert.equal(settlement.ghostWrecksScanned, 1);
    assert.equal(settlement.creatureRemainsScanned, 1);
    assert.equal(settlement.restlessDeadRisen, 1);
    const remains = store.database.prepare(`
      SELECT * FROM ghost_wrecks WHERE source_creature_id = ?
    `).get(creatureId);
    assert.equal(remains.source_vehicle_id, null);
    assert.equal(remains.outcome, 'risen');
    const ghost = store.database.prepare(`
      SELECT * FROM ghost_vehicles WHERE source_creature_id = ?
    `).get(creatureId);
    assert.ok(ghost);
    assert.equal(ghost.source_vehicle_id, null);
    assert.equal(ghost.ghost_kind, scenario.ghostKind);
    assert.equal(ghost.route_id, route.id);
    assert.equal(remains.ghost_id, ghost.id);
    const visible = store.worldEventStatus(saved.id, pursuit.encounterAt).ghosts
      .find((entry) => entry.id === ghost.id);
    assert.equal(visible.sourceCreatureType, scenario.type);
    assert.equal(visible.sourceCreatureId, creatureId);
    assert.equal(visible.icon, catalog.settings.world_creature_icons[scenario.type]);
    const report = store.recentMessages(saved.id, 'all', 'Vehicle')
      .find((message) => message.details.creatureId === creatureId);
    assert.equal(report.details.creatureRemainsId, remains.id);
    assert.match(report.body, /remains may also join the restless dead/i);
    assert.equal(store.settleWorldEvents(pursuit.encounterAt + 1).ghostWrecksScanned, 0);
    assert.equal(store.database.prepare(`
      SELECT COUNT(*) AS count FROM ghost_vehicles WHERE source_creature_id = ?
    `).get(creatureId).count, 1);
  }
});
...
test('spawns every world-creature species in every Common-through-Legendary rarity', (context) => {
  ...
  for (const [type, behavior] of creatureTypes) {
    ...
    for (const tier of tiers) {
      ...
      const ordinaryVehicles = catalog.vehicles.filter((vehicle) =>
        vehicle.routeType === routeTypeId(behavior)
          && catalog.byId.get(vehicle.itemId)?.rarity === tier.id);
      const median = (values) => {
      ...
      const ordinaryDamage = behavior === 'land'
        ? median(ordinaryVehicles.map((vehicle) => Math.max(
          tier.id * 4, vehicle.land.attack * 8
        ))) : tier.id * 4;
      const configuredHealth = Math.round(
        catalog.settings.world_creature_hp[type]
          * catalog.settings.world_creature_tier_hp_multipliers[tier.id]
          * catalog.settings.world_creature_health_factor
      );
      assert.equal(row.max_hp, Math.min(configuredHealth, Math.round(
        ordinaryDamage * catalog.settings.world_creature_endurance[type]
          * catalog.settings.world_creature_health_factor
      )), 'health is capped by the output of ordinary same-tier transports');
...
test('halves existing creature health once and prevents creatures from healing', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-creature-health-'));
  const databaseFile = path.join(directory, 'creature-health.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    try { store.close(); } catch {}
    fs.rmSync(directory, { recursive: true, force: true });
  ...
  const creatureId = Number(store.database.prepare(`
    INSERT INTO world_creatures
      (creature_type, rarity, map_id, route_id, location, destination_city_id,
       hp, max_hp, awakened_at, moved_at, speed)
    VALUES ('land_whale', 1, ?, ?, ?, ?, 80, 140, 2000, 2000, 12)
  `).run(mapId, route.id, route.length / 2, route.city1Id).lastInsertRowid);
  store.database.prepare(
    "DELETE FROM catalog_settings WHERE key = 'world_creature_health_factor'"
  ).run();
  store.close();
  ...
  `).get(creatureId) }, { hp: 4, max_hp: 7 });
  assert.throws(() => store.database.prepare(`
    UPDATE world_creatures SET hp = 12 WHERE id = ?
  `).run(creatureId), /World creatures cannot heal/);
  store.database.prepare('UPDATE world_creatures SET hp = 3 WHERE id = ?').run(creatureId);
  const livingHealthMigration = store.database.prepare(`
    SELECT details_json FROM schema_migrations
    WHERE name = 'living-world-creature-health-third-v1'
  `).get();
  assert.deepEqual(JSON.parse(livingHealthMigration.details_json), {
    factor: 1 / 3, settingInserted: 1, creaturesRebalanced: 1,
  ...
  const proportionalMigration = store.database.prepare(`
    SELECT details_json FROM schema_migrations
    WHERE name = 'proportional-world-creature-balance-v1'
  `).get();
  assert.deepEqual(JSON.parse(proportionalMigration.details_json), {
    healthModel: 'ordinary-transport', creaturesRebalanced: 1,
...
test('sea creatures counterattack every cannon round and skip boarding', (context) => {
  ...
  const pursuit = launchVehicleAtCreature(store, saved.id, creatureId, vehicleId, 5000);
  store.settleWorldEvents(pursuit.encounterAt);
  const report = store.recentMessages(saved.id, 'all', 'Vehicle')
    .find((message) => message.details.creatureId === creatureId);
...
test('a PvE stance launches a ghost hunt across its combat class at reduced force', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  ...
  const ghostCombatBonus = JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'ghost_combat_bonus'
  `).get().value_json);
  const wraithBaseArmorFactor = JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'wraith_base_armor_factor'
  `).get().value_json);
  assert.deepEqual(visibleGhost.landStats, {
    ...unscaledGhostStats,
    armor: Math.round(unscaledGhostStats.armor * wraithBaseArmorFactor
      * (1 + ghostCombatBonus))
  });
  assert.equal(visibleGhost.baseArmorFactor, 1 / 3);
  assert.equal(visibleGhost.combatBonus, ghostCombatBonus);
  ...
  assert.deepEqual(visibleGhost.attackOptions, [],
    'world-event records must not expose direct hunt shortcuts');
  assert.throws(
    () => store.attackGhost(hunter.id, ghost.id, grant.vehicle_id, 3002),
    /Direct hunts have ended/u
  );
  store.setVehicleStance(hunter.id, grant.vehicle_id, 'peaceful', false, true);
  const departure = store.sendVehicle(hunter.id, grant.vehicle_id, route.id, 3002);
  assert.equal(departure.huntsLaunched, 1);
  assert.equal(departure.ghostHuntsLaunched, 1);
  assert.equal(departure.creatureHuntsLaunched, 0);
  const plannedEncounter = store.database.prepare(`
    SELECT encounter_at, encounter_location FROM vehicle_encounters
    WHERE status = 'planned'
      AND ((vehicle1_id = ? AND vehicle2_id = ?)
        OR (vehicle1_id = ? AND vehicle2_id = ?))
    ORDER BY id DESC LIMIT 1
  `).get(grant.vehicle_id, ghost.vehicleId, ghost.vehicleId, grant.vehicle_id);
  ...
  assert.equal(battleDetails.starting[ghostSide].armor, visibleGhost.landStats.armor);
  assert.ok(
...
test('an unequipped spectral Unicycle gains reinforcement without impossible fittings',
  (context) => {
    const store = new SqliteStore(':memory:');
    context.after(() => store.close());
    store.seedCatalog(catalog);
    store.ensureWorldMaps(1000);
    const owner = store.addPlayer(createPlayer(
      'Spectral Unicycle Witness', '', 'hash', catalog, 1000, () => 0.5
    ));
    const unicycle = store.database.prepare(`
      SELECT catalog_vehicles.id AS vehicle_type_id, catalog_vehicles.item_id,
        catalog_items.rarity, catalog_lands.armor
      FROM catalog_vehicles
      JOIN catalog_items ON catalog_items.id = catalog_vehicles.item_id
      JOIN catalog_lands ON catalog_lands.vehicle_id = catalog_vehicles.id
      WHERE catalog_items.name = 'Unicycle'
    `).get();
    const route = store.database.prepare(`
      SELECT id, city1_id, city2_id, length FROM catalog_routes
      WHERE type = ? AND is_open = 1 AND length > 0 AND city1_id <> city2_id
      ORDER BY id LIMIT 1
    `).get(catalog.settings.route_type_ids.land);
    assert.ok(unicycle && route);
    for (const cityId of [route.city1_id, route.city2_id]) store.database.prepare(`
      INSERT OR IGNORE INTO known_cities (player_id, city_id) VALUES (?, ?)
    `).run(owner.id, cityId);
    for (const [key, value] of [
      ['ghost_rise_chance', 1],
      ['ghost_rise_moon_multipliers', Array(8).fill(1)],
      ['ghost_max_active_per_route_class', 99]
    ]) store.database.prepare(
      'UPDATE catalog_settings SET value_json = ? WHERE key = ?'
    ).run(JSON.stringify(value), key);
    store.database.prepare(`
      INSERT INTO ghost_wrecks
        (source_vehicle_id, source_player_id, vehicle_type_id, source_item_id,
         source_vehicle_name, ghost_kind, route_id, location, rarity,
         loadout_json, cause, wrecked_at)
      VALUES (990001, ?, ?, ?, 'Unicycle', 'rider', ?, ?, ?, '{}',
        'spectral balance test', 1500)
    `).run(owner.id, unicycle.vehicle_type_id, unicycle.item_id,
      route.id, route.length / 2, unicycle.rarity);

    const settlement = store.settleWorldEvents(2000);
    assert.equal(settlement.wraithsRisen, 1);
    const ghost = store.worldEventStatus(owner.id, 2001).ghosts.find(
      (entry) => entry.sourceItemId === unicycle.item_id && !entry.defeatedAt
    );
    assert.ok(ghost);
    assert.equal(store.database.prepare(
      'SELECT COUNT(*) AS count FROM player_vehicle_mods WHERE vehicle_id = ?'
    ).get(ghost.vehicleId).count, 0);
    assert.equal(store.database.prepare(
      'SELECT COUNT(*) AS count FROM player_vehicle_weapons WHERE vehicle_id = ?'
    ).get(ghost.vehicleId).count, 0);
    assert.equal(unicycle.armor, 8);
    assert.equal(ghost.landStats.armor, 3);

    const armorMod = store.database.prepare(`
      SELECT catalog_mods.id, catalog_mods.armor FROM catalog_mods
      WHERE catalog_mods.armor > 0 ORDER BY catalog_mods.id LIMIT 1
    `).get();
    assert.ok(armorMod);
    store.database.prepare(
      'INSERT INTO player_vehicle_mods (vehicle_id, mod_id) VALUES (?, ?)'
    ).run(ghost.vehicleId, armorMod.id);
    const fittedGhost = store.worldEventStatus(owner.id, 2002).ghosts.find(
      (entry) => entry.id === ghost.id
    );
    const ghostCombatBonus = JSON.parse(store.database.prepare(`
      SELECT value_json FROM catalog_settings WHERE key = 'ghost_combat_bonus'
    `).get().value_json);
    assert.equal(fittedGhost.landStats.armor,
      Math.round((unicycle.armor / 3 + armorMod.armor) * (1 + ghostCombatBonus)),
      'recovered armour fittings retain their full contribution');
  });
...
test('lets a ship intercept a traveling Kraken and gives bounty only to its killer', (context) => {
  ...
  const pursuit = launchVehicleAtCreature(store, saved.id, creatureId, vehicleId, 3000);
  const rivalPursuit = launchVehicleAtCreature(
    store, savedRival.id, creatureId, rivalVehicleId, 3000
  );
...
test('lets Fisherman ships replace bait with equal-rarity fish while underway', (context) => {
  ...
  const quietRevision = store.latestLiveUpdateId();
  store.vehicleDetails(player.id, vehicleId, 2001);
  assert.equal(store.liveUpdatesAfter(quietRevision).filter((event) =>
    event.scope === `player:${player.id}` || event.scope === 'topic:vehicles').length, 0,
  'checking a travelling ship before its next fishing point must remain read-only');
...
test('purchases each cheaper container once with evenly distributed capacity', (context) => {
  ...
  for (const container of catalog.containers.filter((entry) => entry.id !== chest.id)) {
    store.buyContainer(player.id, container.id);
  }
  assert.equal(store.playerById(player.id).baseItemLimit, 1000,
    'one of every container type reaches the absolute capacity ceiling');
  const warehouse = catalog.gadgetByBehaviorKey.get('warehouse');
  store.database.prepare(`
    INSERT INTO player_gadgets (player_id, gadget_id, expires_at) VALUES (?, ?, ?)
  `).run(player.id, warehouse.id, 5000);
...
test('resolves aggressive land encounters and records ratings and battle reports', (context) => {
  ...
  assert.equal(store.battleReport(highwayman.id, encounter.battleId).opponent.vehicle_name,
    'Live Battle Vehicle');
  const survivingOpponent = store.database.prepare(
    'SELECT arrives_at FROM player_vehicles WHERE id = ?'
  ).get(report.opponentVehicleId);
  if (survivingOpponent) {
    store.vehiclesForPlayer(report.opponent.player_id, survivingOpponent.arrives_at);
...
test('scans a destroyed road vehicle once without guaranteeing a Ghost Rider', (context) => {
  ...
  for (const [key, value] of [
    ...
    ['vehicle_combat_port_safe_zone_max_distance', 0]
  ]) store.database.prepare(`
    UPDATE catalog_settings SET value_json = ? WHERE key = ?
  `).run(JSON.stringify(value), key);
  const vehicleType = catalog.vehicles.find((vehicle) => vehicle.routeType === 0
    ...
    && catalog.routes.some((route) => route.open && route.type === 0
      && route.city1Id !== route.city2Id && (route.city1Id === 1 || route.city2Id === 1)));
  assert.ok(vehicleType);
  const peaceful = store.addPlayer(createPlayer(
    'Future Ghost Rider', '', 'hash', catalog, 1000, () => 0.5
  ));
  const aggressor = store.addPlayer(createPlayer(
    'Wreck Maker', '', 'hash', catalog, 1000, () => 0.5
  ));
  peaceful.inventory[vehicleType.itemId] = 1;
  aggressor.inventory[vehicleType.itemId] = 1;
  store.savePlayer(peaceful);
  store.savePlayer(aggressor);
  const peacefulVehicle = store.activateVehicle(peaceful.id, vehicleType.itemId);
  const aggressiveVehicle = store.activateVehicle(aggressor.id, vehicleType.itemId);
  const route = store.routesForVehicle(peaceful.id, peacefulVehicle, 2000)[0];
  store.sendVehicle(peaceful.id, peacefulVehicle, route.id, 2000);
  store.sendVehicle(aggressor.id, aggressiveVehicle, route.id, 2000, {
    aggressiveMask: 1 << catalog.byId.get(vehicleType.itemId).rarity
  });
  const planned = store.database.prepare(`
    SELECT encounter_at, encounter_location FROM vehicle_encounters
    WHERE status = 'planned' ORDER BY id LIMIT 1
  `).get();
  assert.ok(planned);
  const dwarfTier = catalog.dwarfTiers.find((tier) =>
    tier.rarity === catalog.byId.get(vehicleType.itemId).rarity);
  const stowawayByVehicle = new Map([peacefulVehicle, aggressiveVehicle].map((vehicleId) => [
    vehicleId,
    Number(store.database.prepare(`
      INSERT INTO dwarf_stowaways
        (rarity, item_id, vehicle_id, host_player_id, status, created_at, attached_at)
      VALUES (?, ?, ?, ?, 'aboard', 1500, 2000)
    `).run(dwarfTier.rarity, dwarfTier.itemId, vehicleId,
      vehicleId === peacefulVehicle ? peaceful.id : aggressor.id).lastInsertRowid)
  ]));

  const encounter = resolveNextVehicleEncounter(store);
  const report = store.battleReport(aggressor.id, encounter.battle_id);
  const destroyedSides = report.details.result.ending
    .map((ending, index) => ({ ending, index }))
    .filter(({ ending }) => ending.armor === 0 && ending.reinforcement === 0);
  assert.equal(destroyedSides.length, 1);
  const destroyedIndex = destroyedSides[0].index;
  const destroyedVehicleId = report.details.vehicleIds[destroyedIndex];
  const destroyedPlayerId = destroyedVehicleId === peacefulVehicle
    ? peaceful.id : aggressor.id;
  assert.equal(store.database.prepare(
    'SELECT 1 FROM player_vehicles WHERE id = ?'
  ...
  `).get(escapedStowawayId);
  assert.equal(escapedStowaway.status, 'escaped');
  assert.equal(escapedStowaway.arrival_at, planned.encounter_at);
  assert.deepEqual(combatMessage.details.escapedStowaways.map((entry) => entry.id),
    [escapedStowawayId]);
  assert.match(combatMessage.body, /Dwarf stowaway escaped from the wreck/i);
...
test('keeps Oil Field actions city-local while allowing another city to be selected', (context) => {
  ...
  assert.equal(store.oilField(owner.id, 1000).stats, null,
    'the retired Ledger report must not survive as a hidden Autoloader bonus');
});
...
test('keeps each regional Oil Field independent and blocks remote operation', (context) => {
  ...
  store.ensureWorldMaps(1000);
  const live = store.loadCatalog();
  assert.deepEqual(store.oilFieldCityIds().map((cityId) =>
    live.cities.find((city) => city.id === cityId)?.name), [
    'Burgundy', 'Cinderwake', 'Stormcrag', 'Frostmere'
  ], 'map markers come from the four actual field cities, including non-capital Burgundy');
  const pump = oilMachine('pump', 1);
...
test('issues every existing miner a one-time Cinderwake Oil Field test kit', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  const legacyCatalog = loadLegacyCatalog();
  store.seedCatalog(legacyCatalog);
  const first = store.addPlayer(createPlayer(
    'Cinderwake Tester One', '', hashPassword('field test password one'),
    legacyCatalog, 1000, () => 0.5
  ));
  const second = store.addPlayer(createPlayer(
    'Cinderwake Tester Two', '', hashPassword('field test password two'),
    legacyCatalog, 1001, () => 0.5
  ));

  store.ensureWorldMaps(2000);
  store.provisionCinderwakeOilFieldTestKits(2000);
  const city = store.database.prepare(`
    SELECT catalog_cities.id FROM catalog_cities
    JOIN oil_hexes ON oil_hexes.city_id = catalog_cities.id
    WHERE catalog_cities.name = 'Cinderwake' COLLATE NOCASE
    GROUP BY catalog_cities.id
  `).get();
  assert.ok(city);
  const expectedMachineItems = store.database.prepare(`
    SELECT catalog_items.id, catalog_items.rarity,
      json_extract(catalog_machine_types.rules_json, '$.isBomb') AS is_bomb
  ...
  `).all();
  assert.equal(expectedMachineItems.length, 46);

  for (const player of [first, second]) {
    const grantedMachines = store.database.prepare(`
      SELECT inventory.item_id, inventory.quantity
      FROM inventory JOIN catalog_machines ON catalog_machines.item_id = inventory.item_id
      WHERE inventory.player_id = ? AND inventory.city_id = ?
      ORDER BY inventory.item_id
    `).all(player.id, city.id);
    assert.deepEqual(grantedMachines.map((item) => ({ ...item })), expectedMachineItems.map((item) => ({
      item_id: item.id,
      quantity: item.is_bomb ? 4 : Number(item.rarity) === 1 ? 2 : 1
    })));
    for (const setting of ['search_plane_item_id', 'bomber_item_id', 'helicopter_item_id']) {
      const itemId = JSON.parse(store.database.prepare(
        'SELECT value_json FROM catalog_settings WHERE key = ?'
      ).get(setting).value_json);
      assert.equal(store.database.prepare(`
        SELECT quantity FROM inventory
        WHERE player_id = ? AND city_id = ? AND item_id = ?
      `).get(player.id, city.id, itemId).quantity, 1);
    }
    const message = store.recentMessages(player.id, 'all', 'Machine')
      .find((entry) => entry.details.event === 'cinderwake-oil-field-test-kit');
    assert.equal(message.senderName, 'The Council');
    assert.equal(message.details.cityId, city.id);
    assert.equal(message.details.grants.machineItems.length, 46);
    assert.deepEqual(message.actionLinks.map((action) => action.href), [
      '/oil-field', '/gadgets', `/inventory?city=${city.id}`
    ]);
  }

  const inventoryBefore = store.database.prepare(`
    SELECT SUM(quantity) AS quantity FROM inventory
    WHERE player_id IN (?, ?) AND city_id = ?
  `).get(first.id, second.id, city.id).quantity;
  store.ensureWorldMaps(3000);
  assert.equal(store.database.prepare(`
    SELECT SUM(quantity) AS quantity FROM inventory
    WHERE player_id IN (?, ?) AND city_id = ?
  `).get(first.id, second.id, city.id).quantity, inventoryBefore);
  const migration = store.database.prepare(`
    SELECT applied_at, details_json FROM schema_migrations WHERE name = ?
  `).get('cinderwake-oil-field-test-kits-v1');
  assert.equal(migration.applied_at, 2000);
  assert.deepEqual(JSON.parse(migration.details_json), {
    cityId: city.id, cityName: 'Cinderwake', players: 2,
    distinctMachineItems: 46, distinctSupportItems: 4,
    quantityPerPlayer: 65
  });
});
...
test('fitted Magnets salvage passed wreckage once each per journey and clear empty wrecks',
  (context) => {
    const store = new SqliteStore(':memory:');
    context.after(() => store.close());
    store.seedCatalog(catalog);
    ...
    store.database.prepare(`
      UPDATE catalog_settings SET value_json = '1'
      WHERE key = 'magnet_intact_salvage_chance'
    `).run();
    const route = catalog.routes.find((entry) => entry.open && entry.type === 0
      && entry.city1Id !== entry.city2Id && [entry.city1Id, entry.city2Id].includes(1));
    const vehicleType = catalog.vehicles.find((entry) => entry.routeType === 0
      && entry.capacity >= 2 && catalog.byId.get(entry.itemId)?.rarity === 1);
    const damagedVehicle = catalog.items.find(
      (item) => item.repairedItemId === vehicleType?.itemId
    );
    const looseThing = catalog.items.find((item) => item.rarity === 1
      && item.id !== vehicleType?.itemId && !catalog.vehicleByItemId.has(item.id)
      && !catalog.weaponByItemId.has(item.id) && !catalog.cannonByItemId.has(item.id)
      && !catalog.cannonballByItemId.has(item.id));
    assert.ok(route && vehicleType && damagedVehicle && looseThing);
    const driver = store.addPlayer(createPlayer(
      'Magnetic Driver', '', 'hash', catalog, 1000, () => 0.5
    ));
    driver.inventory[vehicleType.itemId] = 1;
    driver.inventory[MAGNET_CATALOG.items[0].id] = 1;
    const boltItemId = Number(catalog.settings.bolt_item_id);
    const boltsBefore = driver.inventory[boltItemId] ?? 0;
    driver.inventory[boltItemId] = boltsBefore + 1;
    store.savePlayer(driver);
    const vehicleId = store.activateVehicle(driver.id, vehicleType.itemId, 1000);
    const cargoPreview = store.previewVehicleCargo(driver.id, vehicleId, {
      [MAGNET_CATALOG.items[0].id]: 1
    });
    assert.equal(cargoPreview.valid, false);
    assert.match(cargoPreview.reasons.join(' '), /fitted with a Bolt/u);
    const fittingPreview = store.previewVehicleFittings(
      driver.id, vehicleId, [MAGNET_CATALOG.mod.id], []
    );
    assert.equal(fittingPreview.valid, true);
    assert.equal(fittingPreview.boltsRequired, 1);
    store.fitVehicleLoadout(driver.id, vehicleId, [MAGNET_CATALOG.mod.id], []);
    assert.equal(store.vehicleDetails(driver.id, vehicleId, 1000).mods[0].itemId,
      MAGNET_CATALOG.items[0].id);
    assert.equal(store.playerById(driver.id).inventory[boltItemId] ?? 0, boltsBefore);
    const wreckLocation = Number(route.length) / 3;
    const insertWreck = (sourceVehicleId, itemIds, wreckedAt) => {
      store.database.prepare(`
        INSERT INTO ghost_wrecks
          (source_vehicle_id, source_player_id, vehicle_type_id, source_item_id,
           source_vehicle_name, ghost_kind, route_id, location, rarity,
           loadout_json, cause, wrecked_at)
        VALUES (?, ?, ?, ?, 'Test wreck', 'rider', ?, ?, 1, '{}', 'test', ?)
      `).run(sourceVehicleId, driver.id, vehicleType.id, vehicleType.itemId,
        route.id, wreckLocation, wreckedAt);
      ...
      for (const itemId of itemIds) {
        insertPart.run(itemId, route.id, wreckLocation, sourceVehicleId, wreckedAt);
      }
    };
    insertWreck(900001, [looseThing.id, damagedVehicle.id], 1500);

    const firstRoute = store.routesForVehicle(driver.id, vehicleId, 2000)
      .find((entry) => entry.id === route.id);
    const firstTrip = store.sendVehicle(driver.id, vehicleId, firstRoute.id, 2000);
    store.vehicleDetails(driver.id, vehicleId, firstTrip.arrivesAt + 1);
    assert.equal(store.playerById(driver.id).inventoryByCity[firstTrip.destinationCityId][
      looseThing.id
    ], 1);
    assert.equal(store.database.prepare(`
      SELECT COUNT(*) AS count FROM sunken_items WHERE source_vehicle_id = 900001
    `).get().count, 1, 'one fitted Magnet makes one pull in a journey');
    assert.ok(store.database.prepare(
      'SELECT 1 FROM ghost_wrecks WHERE source_vehicle_id = 900001'
    ).get(), 'a wreck remains while it still contains a part');

    const secondRoute = store.routesForVehicle(driver.id, vehicleId, firstTrip.arrivesAt + 2)
      .find((entry) => entry.id === route.id);
    const secondTrip = store.sendVehicle(
      driver.id, vehicleId, secondRoute.id, firstTrip.arrivesAt + 2
    );
    store.vehicleDetails(driver.id, vehicleId, secondTrip.arrivesAt + 1);
    assert.equal(store.playerById(driver.id).inventoryByCity[secondTrip.destinationCityId][
      damagedVehicle.id
    ], 1, 'the damaged vehicle chassis can survive a Magnet pull');
    assert.equal(store.database.prepare(
      'SELECT 1 FROM ghost_wrecks WHERE source_vehicle_id = 900001'
    ).get(), undefined, 'the exhausted wreck is removed');

    store.database.prepare(`
      UPDATE catalog_settings SET value_json = '0'
      WHERE key = 'magnet_intact_salvage_chance'
    `).run();
    insertWreck(900002, [looseThing.id], secondTrip.arrivesAt + 2);
    const thirdRoute = store.routesForVehicle(driver.id, vehicleId, secondTrip.arrivesAt + 3)
      .find((entry) => entry.id === route.id);
    const thirdTrip = store.sendVehicle(
      driver.id, vehicleId, thirdRoute.id, secondTrip.arrivesAt + 3
    );
    store.vehicleDetails(driver.id, vehicleId, thirdTrip.arrivesAt + 1);
    const expectedScraps = Number(catalog.settings.recycling_scraps_by_rarity[looseThing.rarity]);
    assert.equal(store.database.prepare(`
      SELECT quantity FROM recycling_scraps WHERE player_id = ? AND city_id = ?
    `).get(driver.id, thirdTrip.destinationCityId).quantity, expectedScraps);
    assert.equal(store.database.prepare(
      'SELECT 1 FROM ghost_wrecks WHERE source_vehicle_id = 900002'
    ).get(), undefined);
    const events = store.database.prepare(`
      SELECT details_json FROM vehicle_events
      WHERE vehicle_id = ? AND event_type = 'magnet-salvage'
      ORDER BY id
    `).all(vehicleId).map((entry) => JSON.parse(entry.details_json));
    assert.deepEqual(events.map((event) => event.outcome), ['cargo', 'cargo', 'scraps']);
    assert.ok(events.every((event) => event.wreckCleared === (event !== events[0])));
    assert.ok(store.stonesForPlayer(driver.id).earned.some(
      (stone) => stone.behaviorKey === 'Magnetised'
    ));
  });
...
test('Autoloader supplies a configured shuttle after landing and before it departs again',
  (context) => {
    const store = new SqliteStore(':memory:');
    context.after(() => store.close());
    store.seedCatalog(catalog);
    store.ensureWorldMaps(1000);
    const route = store.database.prepare(`
      SELECT catalog_routes.*, world_maps.capital_city_id,
        CASE WHEN catalog_routes.city1_id = world_maps.capital_city_id
          THEN catalog_routes.city2_id ELSE catalog_routes.city1_id END AS outpost_city_id
      FROM catalog_routes
      JOIN catalog_cities AS first_city ON first_city.id = catalog_routes.city1_id
      JOIN world_maps ON world_maps.id = first_city.map_id
        AND world_maps.capital_city_id IN
          (catalog_routes.city1_id, catalog_routes.city2_id)
      WHERE catalog_routes.is_open = 1 AND catalog_routes.type = 1
        AND catalog_routes.city1_id <> catalog_routes.city2_id
      ORDER BY catalog_routes.id LIMIT 1
    `).get();
    const shipDefinition = catalog.ships.find((candidate) => {
      const vehicle = catalog.vehicles.find((entry) => entry.id === candidate.vehicleId);
      return candidate.cannonPortals > 0 && vehicle?.routeType === 1 && vehicle.capacity >= 3;
    });
    const shipType = catalog.vehicles.find((entry) => entry.id === shipDefinition?.vehicleId);
    const allowed = catalog.settings.arms_rarities_by_vehicle_rarity[
      catalog.byId.get(shipType?.itemId)?.rarity
    ] ?? [];
    const cannon = catalog.cannons.find((entry) =>
      allowed.includes(catalog.byId.get(entry.itemId).rarity));
    const ammunition = catalog.cannonballs[0];
    const cargo = catalog.byId.get(WOOD_CATALOG.items.find((item) => item.rarity === 1).id);
    assert.ok(route && shipType && cannon && ammunition && cargo);

    const originCityId = Number(route.capital_city_id);
    const destinationCityId = Number(route.outpost_city_id);
    const draft = createPlayer('Shuttle Autoloader', '', 'hash', catalog, 1000, () => 0.5);
    draft.cityId = originCityId;
    draft.homeCityId = originCityId;
    draft.mines[0].cityId = originCityId;
    draft.inventory = { [shipType.itemId]: 1, [cannon.itemId]: 1, [cargo.id]: 100 };
    draft.inventoryByCity = { [originCityId]: draft.inventory };
    const player = store.addPlayer(draft);
    const vehicleId = store.activateVehicle(player.id, shipType.itemId);
    store.attachShipCannon(player.id, vehicleId, cannon.id);
    ...
    store.database.prepare(`
      INSERT INTO player_gadgets (player_id, gadget_id, expires_at) VALUES (?, ?, 9999999999)
    `).run(player.id, autoloader.id);
    store.database.prepare(
      'INSERT OR IGNORE INTO known_cities (player_id, city_id) VALUES (?, ?)'
    ).run(player.id, destinationCityId);
    assert.throws(() => store.configureGadgetAutomation(player.id, 'autoloader', {
      vehicleId, cityId: destinationCityId, ammoType: ammunition.type,
      quantity: 1, intervalMinutes: 5
    }, 1700), /must use a regional capital/u);
    store.configureGadgetAutomation(player.id, 'autoloader', {
      vehicleId, cityId: originCityId, ammoType: ammunition.type,
      quantity: 1, intervalMinutes: 5
    }, 1800);

    store.startVehicleShuttle(
      player.id, vehicleId, route.id, 2000, [WOOD_CATALOG.mineType.id]
    );
    stockShuttleThing(store, player.id, originCityId, ammunition.itemId, 3);
    stockShuttleThing(store, player.id, destinationCityId, ammunition.itemId, 2);
    let vehicle = store.database.prepare(
      'SELECT * FROM player_vehicles WHERE id = ?'
    ).get(vehicleId);
    store.settleVehicles(Number(vehicle.arrives_at));
    vehicle = store.database.prepare('SELECT * FROM player_vehicles WHERE id = ?').get(vehicleId);
    assert.equal(Number(vehicle.origin_city_id), destinationCityId);
    assert.equal(Number(vehicle.destination_city_id), originCityId);
    const ammoField = catalog.settings.ammunition_rules[ammunition.type].storageField;
    assert.equal(store.database.prepare(
      `SELECT ${ammoField} AS shots FROM player_ship_state WHERE vehicle_id = ?`
    ).get(vehicleId).shots, 0, 'the outpost turnaround must not load ammunition');
    assert.equal(shuttleInventoryQuantity(
      store, player.id, destinationCityId, ammunition.itemId
    ), 2, 'ammunition stored outside the capital must remain untouched');

    store.settleVehicles(Number(vehicle.arrives_at));
    vehicle = store.database.prepare('SELECT * FROM player_vehicles WHERE id = ?').get(vehicleId);
    assert.equal(vehicle.status, 'traveling');
    assert.equal(Number(vehicle.origin_city_id), originCityId);
    assert.equal(Number(vehicle.destination_city_id), destinationCityId);
    assert.equal(store.database.prepare(
      `SELECT ${ammoField} AS shots FROM player_ship_state WHERE vehicle_id = ?`
    ).get(vehicleId).shots, Number(catalog.settings.shots_per_crate));
    assert.equal(shuttleInventoryQuantity(
      store, player.id, originCityId, ammunition.itemId
    ), 2);
    assert.match(store.database.prepare(`
      SELECT last_status FROM gadget_automations WHERE player_id = ? AND gadget_id = ?
    `).get(player.id, autoloader.id).last_status, /Shuttle turnaround: Loaded 1 crate/u);
  });

test('a shuttle persists one selected mine category and leaves other stock behind', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  const rig = shuttleTestRig(store, 'Single-category Shuttle');
  const wood = catalog.byId.get(WOOD_CATALOG.items.find((item) => item.rarity === 6).id);
  const wisdom = catalog.byId.get(WISDOM_CATALOG.items.find((item) => item.rarity === 6).id);
  assert.ok(wood && wisdom);
  stockShuttleThing(store, rig.player.id, rig.originCityId, wood.id, 3);
  stockShuttleThing(store, rig.player.id, rig.originCityId, wisdom.id, 3);

  const started = store.startVehicleShuttle(
    rig.player.id, rig.vehicleId, rig.route.id, 2000, [WOOD_CATALOG.mineType.id]
  );

  assert.deepEqual(started.shuttle.mineTypeIds, [WOOD_CATALOG.mineType.id]);
  assert.equal(store.database.prepare(`
    SELECT mine_type_ids_json FROM player_vehicle_shuttles WHERE vehicle_id = ?
  `).get(rig.vehicleId).mine_type_ids_json, JSON.stringify([WOOD_CATALOG.mineType.id]));
  assert.deepEqual(store.vehicleDetails(rig.player.id, rig.vehicleId, 2001).cargo
    .map((entry) => [entry.itemId, entry.quantity]), [[wood.id, 3]]);
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.originCityId, wisdom.id
...
test('multiple shuttle mine categories are normalized and filled rarest first', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  const rig = shuttleTestRig(store, 'Multi-category Shuttle');
  const wood = catalog.byId.get(WOOD_CATALOG.items.find((item) => item.rarity === 6).id);
  const wisdom = catalog.byId.get(WISDOM_CATALOG.items.find((item) => item.rarity === 5).id);
  const relic = catalog.byId.get(RELICS_CATALOG.items.find((item) => item.rarity === 6).id);
  assert.ok(wood && wisdom && relic);
  stockShuttleThing(store, rig.player.id, rig.originCityId, wood.id, 2);
  stockShuttleThing(store, rig.player.id, rig.originCityId, wisdom.id, 20);
  stockShuttleThing(store, rig.player.id, rig.originCityId, relic.id, 20);

  const started = store.startVehicleShuttle(
    rig.player.id, rig.vehicleId, rig.route.id, 2000,
    [WISDOM_CATALOG.mineType.id, WOOD_CATALOG.mineType.id, WISDOM_CATALOG.mineType.id]
  );

  assert.deepEqual(started.shuttle.mineTypeIds,
  ...
  assert.equal(cargo.get(wood.id), 2);
  assert.equal(cargo.get(wisdom.id), 8,
    'the next-rarest selected category fills the remaining capacity');
  assert.equal(cargo.has(relic.id), false,
    'a rarer thing from an unselected category is not loaded');
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.originCityId, relic.id
  ), 20);
});
...
test('explicit empty or invalid shuttle mine categories are rejected atomically', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  const rig = shuttleTestRig(store, 'Invalid-category Shuttle');
  ...
  assert.equal(store.vehicleDetails(rig.player.id, rig.vehicleId, 2001).status, 'idle');
});
...
test('migrates v112 shuttle contracts as unrestricted mine-category selections', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-shuttle-v112-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  const rig = shuttleTestRig(store, 'Category Migrator');
  store.startVehicleShuttle(rig.player.id, rig.vehicleId, rig.route.id, 2000);
  store.close();
  const legacy = new DatabaseSync(databaseFile);
  legacy.exec(`
    ALTER TABLE player_vehicle_shuttles DROP COLUMN mine_type_ids_json;
  ...
  assert.equal(store.database.prepare(`
    SELECT mine_type_ids_json FROM player_vehicle_shuttles WHERE vehicle_id = ?
  `).get(rig.vehicleId).mine_type_ids_json, null);
  assert.equal(store.vehicleDetails(rig.player.id, rig.vehicleId, 2001).shuttle.mineTypeIds, null);
  const cargo = catalog.byId.get(WOOD_CATALOG.items.find((item) => item.rarity === 6).id);
  stockShuttleThing(store, rig.player.id, rig.originCityId, cargo.id, 2);
  store.settleVehicles(302000);
  assert.equal(store.vehicleDetails(rig.player.id, rig.vehicleId, 302001).cargo
    .find((entry) => entry.itemId === cargo.id)?.quantity, 2,
    'a migrated null selection retains the legacy all-category contract');
});
...
test('an empty shuttle waits for eligible stock and resumes automatically', (context) => {
  ...
  assert.equal(store.database.prepare(
    'SELECT status FROM player_vehicles WHERE id = ?'
  ).get(rig.vehicleId).status, 'idle', 'a paused shuttle is not retried early');
  store.settleVehicles(302000);
  ...
  assert.equal(store.vehicleDetails(rig.player.id, rig.vehicleId, 302001).cargoSize, 4);
});

test('paused shuttles retry no more than once every five minutes', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  ...
  assert.deepEqual(relevantCounts(), beforeRetry,
    'an early maintenance tick must not retry or wake live clients');

  store.settleVehicles(302000);
  const afterDueRetry = relevantCounts();
  assert.equal(afterDueRetry[`player:${rig.player.id}`], beforeRetry[`player:${rig.player.id}`] + 1);
  assert.equal(afterDueRetry['topic:vehicles'], beforeRetry['topic:vehicles'] + 1);
  assert.equal(store.database.prepare(`
    SELECT updated_at FROM player_vehicle_shuttles WHERE vehicle_id = ?
  `).get(rig.vehicleId).updated_at, 302000);
  store.settleVehicles(302001);
  assert.deepEqual(relevantCounts(), afterDueRetry,
    'the next retry is scheduled five minutes after the completed attempt');
});
...
test('a shuttle waits above the configured inventory overage and resumes below it', (context) => {
  ...
  assert.equal(store.vehicleDetails(rig.player.id, rig.vehicleId, 302001).cargoSize, 2);
});
...
test('snow pauses a shuttle turnaround after safely delivering its cargo', (context) => {
  ...
  store.settleVehicles(deliveredAt + 5 * 60 * 1000);
  vehicle = store.database.prepare('SELECT * FROM player_vehicles WHERE id = ?')
    .get(rig.vehicleId);
  shuttle = store.database.prepare(
    'SELECT * FROM player_vehicle_shuttles WHERE vehicle_id = ?'
...
test('shuttles fill from the rarest eligible stock without moving manufactured or Oil Field machine parts', (context) => {
  ...
  const details = store.vehicleDetails(rig.player.id, rig.vehicleId, 2001);
  assert.equal(details.shuttle.mineTypeIds, null,
    'omitting category IDs keeps the legacy all-category contract');
  const cargo = new Map(details.cargo
...
test('a deactivated transport can shuttle as cargo and reactivate at its destination', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  const rig = shuttleTestRig(store, 'Packed Transport Shuttle');
  stockShuttleThing(
    store, rig.player.id, rig.originCityId, rig.vehicleType.itemId, 1
  );
  const packedVehicleId = store.activateVehicle(
    rig.player.id, rig.vehicleType.itemId, 1900
  );

  store.storeVehicle(rig.player.id, packedVehicleId);
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.originCityId, rig.vehicleType.itemId
  ), 1, 'deactivation returns the transport to local inventory');

  store.startVehicleShuttle(rig.player.id, rig.vehicleId, rig.route.id, 2000);
  const outbound = store.database.prepare(
    'SELECT arrives_at FROM player_vehicles WHERE id = ?'
  ).get(rig.vehicleId);
  assert.equal(store.vehicleDetails(rig.player.id, rig.vehicleId, 2001).cargo
    .find((item) => item.itemId === rig.vehicleType.itemId)?.quantity, 1,
  'the packed transport loads into a compatible shuttle');

  store.settleVehicles(Number(outbound.arrives_at));
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.destinationCityId, rig.vehicleType.itemId
  ), 1, 'the packed transport unloads as a Thing at the destination');

  store.database.prepare('UPDATE players SET city_id = ? WHERE id = ?')
    .run(rig.destinationCityId, rig.player.id);
  const reactivatedVehicleId = store.activateVehicle(
    rig.player.id, rig.vehicleType.itemId, Number(outbound.arrives_at) + 1
  );
  assert.equal(store.database.prepare(
    'SELECT city_id FROM player_vehicles WHERE id = ?'
  ).get(reactivatedVehicleId).city_id, rig.destinationCityId);
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.destinationCityId, rig.vehicleType.itemId
  ), 0, 'reactivation consumes the delivered packed Thing');
});
...
test('starting a shuttle requires usable capacity after vehicle fittings', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  const rig = shuttleTestRig(store, 'Fully Fitted Shuttle');
  const weapon = catalog.weapons.find((entry) =>
    catalog.byId.get(entry.itemId)?.rarity === 6);
  const wood = catalog.byId.get(WOOD_CATALOG.items.find((item) => item.rarity === 6).id);
  assert.ok(weapon && wood);
  const insertWeapon = store.database.prepare(`
    INSERT INTO player_vehicle_weapons (vehicle_id, weapon_id) VALUES (?, ?)
  `);
  for (let slot = 0; slot < rig.vehicleType.capacity; slot += 1) {
    insertWeapon.run(rig.vehicleId, weapon.id);
  }
  stockShuttleThing(store, rig.player.id, rig.originCityId, wood.id, 1);
  const before = store.vehicleDetails(rig.player.id, rig.vehicleId, 2000);
  assert.equal(before.cargoSize, 0);
  ...
  assert.throws(
    ...
    /free cargo|cargo capacity|capacity.*cargo/i
  );

  assert.equal(store.vehicleDetails(rig.player.id, rig.vehicleId, 2000).status, 'idle');
  assert.equal(store.database.prepare(`
    SELECT COUNT(*) AS count FROM player_vehicle_shuttles WHERE vehicle_id = ?
  `).get(rig.vehicleId).count, 0, 'a zero-capacity vehicle does not retain a dead contract');
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.originCityId, wood.id
  ), 1, 'eligible stock remains untouched');
});

test('a Wood-only shuttle keeps its category filter when it reloads after returning', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  ...
  const wisdom = catalog.byId.get(WISDOM_CATALOG.items.find((item) => item.rarity === 6).id);
  assert.ok(wood && wisdom);
  stockShuttleThing(store, rig.player.id, rig.originCityId, wood.id, 25);
  stockShuttleThing(store, rig.player.id, rig.originCityId, wisdom.id, 25);
  store.startVehicleShuttle(
    rig.player.id, rig.vehicleId, rig.route.id, 2000, [WOOD_CATALOG.mineType.id]
  );
  let vehicle = store.database.prepare('SELECT * FROM player_vehicles WHERE id = ?')
  ...
  assert.equal(vehicle.origin_city_id, rig.originCityId);
  assert.equal(vehicle.destination_city_id, rig.destinationCityId);
  assert.equal(shuttleInventoryQuantity(store, rig.player.id, rig.originCityId, wood.id), 15);
  assert.equal(shuttleInventoryQuantity(store, rig.player.id, rig.originCityId, wisdom.id), 25);
  ...
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.destinationCityId, wood.id
  ), 10);
  assert.equal(shuttleInventoryQuantity(
    store, rig.player.id, rig.destinationCityId, wisdom.id
  ), 0, 'the unselected category is not delivered on the first trip');
  assert.equal(shuttle.deliveries, 1);
  assert.equal(shuttle.delivered_things, 10);
  ...
  const secondOutbound = store.vehicleDetails(rig.player.id, rig.vehicleId, returnAt);
  assert.equal(secondOutbound.cargoSize, 10,
    'the next outbound journey reloads at the origin');
  assert.deepEqual(secondOutbound.shuttle.mineTypeIds, [WOOD_CATALOG.mineType.id]);
  assert.equal(secondOutbound.cargo.find((entry) => entry.itemId === wood.id)?.quantity, 10);
  assert.equal(secondOutbound.cargo.some((entry) => entry.itemId === wisdom.id), false,
    'the second outbound load still excludes Wisdom');
  assert.equal(shuttleInventoryQuantity(store, rig.player.id, rig.originCityId, wood.id), 5);
  assert.equal(shuttleInventoryQuantity(store, rig.player.id, rig.originCityId, wisdom.id), 25);
  shuttle = store.database.prepare(
    'SELECT * FROM player_vehicle_shuttles WHERE vehicle_id = ?'
  ).get(rig.vehicleId);
...
test('creates an independent ore-thief operation in every region', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  store.ensureWorldMaps(1000);
  ...
  const operations = store.database.prepare(`
    SELECT world_maps.id AS map_id, world_maps.name AS map_name,
      world_maps.capital_city_id, catalog_routes.city1_id AS mission_city_id,
      COUNT(DISTINCT thief_bases.id) AS bases
    FROM world_maps
    JOIN catalog_cities ON catalog_cities.map_id = world_maps.id
    JOIN catalog_routes ON catalog_routes.city1_id = catalog_cities.id
      AND catalog_routes.city2_id = catalog_cities.id
      AND catalog_routes.type = ?
    JOIN thief_bases ON thief_bases.map_id = world_maps.id
    GROUP BY world_maps.id, world_maps.name, world_maps.capital_city_id,
      catalog_routes.city1_id
    ORDER BY world_maps.sort_order, world_maps.id
  `).all(liveCatalog.settings.route_type_ids.air);
  assert.equal(operations.length, liveCatalog.maps.length);
  assert.ok(operations.every((operation) => Number(operation.bases) === 1));
  const aso = operations.find((operation) => operation.map_name === 'Aso');
  const bromo = operations.find((operation) => operation.map_name === 'Bromo');
  assert.equal(Number(aso.mission_city_id), Number(aso.capital_city_id));

  const searchPlane = liveCatalog.vehicles.find((vehicle) =>
    liveCatalog.byId.get(vehicle.itemId)?.name === 'Search Plane');
  const draft = createPlayer(
    'Regional Mission Pilot', '', 'hash', liveCatalog, 1000, () => 0.5
  );
  draft.profession = 10;
  draft.cityId = Number(aso.mission_city_id);
  draft.homeCityId = draft.cityId;
  draft.knownCityIds = [Number(aso.mission_city_id), Number(bromo.mission_city_id)];
  draft.inventoryByCity = {
    [aso.mission_city_id]: { [searchPlane.itemId]: 1 },
    [bromo.mission_city_id]: { [searchPlane.itemId]: 1 }
  };
  draft.inventory = draft.inventoryByCity[draft.cityId];
  const pilot = store.addPlayer(draft);
  const addMeld = store.database.prepare(
    'INSERT INTO player_melds (player_id, meld_id, created_at) VALUES (?, ?, 1000)'
  );
  for (const meld of liveCatalog.melds.slice(0, 20)) addMeld.run(pilot.id, meld.id);
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = '1'
    WHERE key = 'aircraft_search_chance'
  `).run();
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = '0'
    WHERE key = 'aircraft_shot_down_chance'
  `).run();

  const asoPlaneId = store.activateVehicle(pilot.id, searchPlane.itemId);
  const asoRoute = store.routesForVehicle(pilot.id, asoPlaneId, 2000)
    .find((route) => route.mission);
  assert.ok(asoRoute, 'Cinderwake aircraft can launch the Aso mission');
  const asoTrip = store.sendVehicle(pilot.id, asoPlaneId, asoRoute.id, 2000);
  const asoEventAt = store.vehicleDetails(pilot.id, asoPlaneId, 2000).aircraftEventAt;
  store.settleVehicles(asoEventAt);
  assert.equal(store.thiefBaseStatus(pilot.id, asoEventAt).discovered, true);

  store.changeCity(pilot.id, Number(bromo.mission_city_id), asoEventAt + 1);
  const bromoStatus = store.thiefBaseStatus(pilot.id, asoEventAt + 1);
  assert.equal(bromoStatus.discovered, false);
  assert.equal(bromoStatus.mapName, 'Bromo');
  const bromoPlaneId = store.activateVehicle(pilot.id, searchPlane.itemId);
  assert.ok(store.routesForVehicle(pilot.id, bromoPlaneId, asoEventAt + 1)
    .some((route) => route.mission), 'Aso discovery does not close Bromo searching');
  assert.ok(asoTrip.arrivesAt > asoEventAt);
});
...
test('snapshots travel gadgets, applies meld defenses, and reports radar and binocular data', (context) => {
  ...
  assert.ok(store.database.prepare(`
    ...
    WHERE vehicle_id = ? AND battle_id = ? AND other_vehicle_id = ?
  `).get(attackerVehicle, journey.battleId, defenderVehicle));
  assert.ok(store.database.prepare(`
    SELECT 1 FROM vehicle_events
    WHERE vehicle_id = ? AND battle_id = ? AND other_vehicle_id IS NULL
  `).get(defenderVehicle, journey.battleId),
  'binocular privacy is retained in the destroyed vehicle history');
});
...
test('pillages cargo, then oil, then one stealable fitting in the original order', (context) => {
  ...
  `);
  const prepareVictim = (time, { oil = 0, cargo = 0, fitting = false } = {}) => {
    const cityId = store.vehicleDetails(highwayman.id, robberVehicle, time).cityId;
    store.database.prepare(`
      INSERT OR IGNORE INTO known_cities (player_id, city_id) VALUES (?, ?)
    `).run(trader.id, cityId);
    store.changeCity(highwayman.id, cityId, time);
    store.changeCity(trader.id, cityId, time);
    stock.run(trader.id, cityId, traderType.itemId, 1);
    if (fitting) stock.run(trader.id, cityId, defensiveWeapon.itemId, 1);
    if (cargo) stock.run(trader.id, cityId, ordinaryCargo.id, cargo);
    const victimVehicle = store.activateVehicle(trader.id, traderType.itemId, time);
    if (fitting) store.fitVehicleWeapons(trader.id, victimVehicle, [defensiveWeapon.id]);
    if (cargo) store.setVehicleCargo(trader.id, victimVehicle, { [ordinaryCargo.id]: cargo });
    if (oil) store.database.prepare(
      'UPDATE player_vehicles SET oiled_trips = ? WHERE id = ?'
    ).run(oil, victimVehicle);
    return victimVehicle;
  };
  const sendEncounter = (time, victimVehicle) => {
    const route = store.routesForVehicle(trader.id, victimVehicle, time)[0];
    store.sendVehicle(trader.id, victimVehicle, route.id, time);
    ...
    return { arrivesAt: arrivesAt + 1, battleId: robberTrip.battleId };
  };

  store.database.prepare('UPDATE player_vehicles SET oiled_trips = 20 WHERE id = ?')
    .run(robberVehicle);
  let victimVehicle = prepareVictim(2000, { oil: 20, fitting: true });
  let encounter = sendEncounter(2000, victimVehicle);
  let nextTime = encounter.arrivesAt;
  ...
  victimVehicle = prepareVictim(secondStart, { fitting: true });
  encounter = sendEncounter(secondStart, victimVehicle);
  ...
  nextTime = encounter.arrivesAt;
  store.vehiclesForPlayer(highwayman.id, nextTime);
  victimVehicle = prepareVictim(nextTime + 1, { cargo: 3, oil: 20, fitting: true });
  sendEncounter(nextTime + 1, victimVehicle);
...
test('lets a captured Dwarf fight an NPC captor each round and escape when it is defeated',
  (context) => {
    const store = new SqliteStore(':memory:');
    context.after(() => store.close());
    store.seedCatalog(catalog);
    ...
    assert.ok(vehicleType);
    store.database.prepare(`
      UPDATE catalog_lands SET attack = 0, armor = 5 WHERE vehicle_id = ?
    `).run(vehicleType.id);
    const traveler = createPlayer('Dwarf Rescuer', '', 'hash', catalog, 1000, () => 0.5);
    const captor = createPlayer('Test NPC Captor', '', 'hash', catalog, 1000, () => 0.5);
    traveler.profession = 2;
    captor.profession = 2;
    traveler.inventory[vehicleType.itemId] = 1;
    captor.inventory[vehicleType.itemId] = 1;
    const savedTraveler = store.addPlayer(traveler);
    const savedCaptor = store.addPlayer(captor);
    store.database.prepare('UPDATE players SET is_npc = 1 WHERE id = ?').run(savedCaptor.id);
    const travelerVehicle = store.activateVehicle(savedTraveler.id, vehicleType.itemId);
    const captorVehicle = store.activateVehicle(savedCaptor.id, vehicleType.itemId);
    const dwarf = catalog.dwarfTiers.find((tier) => tier.rarity === 3);
    const stowawayId = Number(store.database.prepare(`
      INSERT INTO dwarf_stowaways
        (rarity, item_id, vehicle_id, host_player_id, captor_player_id,
         status, created_at, attached_at)
      VALUES (?, ?, ?, ?, ?, 'captured', 1500, 1600)
    `).run(dwarf.rarity, dwarf.itemId, captorVehicle,
      savedTraveler.id, savedCaptor.id).lastInsertRowid);
    store.database.prepare(`
      INSERT INTO player_vehicle_cargo (vehicle_id, item_id, quantity) VALUES (?, ?, 1)
    `).run(captorVehicle, dwarf.itemId);
    const route = store.routesForVehicle(savedTraveler.id, travelerVehicle, 2000)[0];
    assert.ok(route);
    const travelerTrip = store.sendVehicle(savedTraveler.id, travelerVehicle, route.id, 2000);
    const captorTrip = store.sendVehicle(savedCaptor.id, captorVehicle, route.id, 2000, {
      aggressiveMask: 1 << catalog.byId.get(vehicleType.itemId).rarity
    });
    store.database.prepare(`
      ...
      VALUES (?, ?, ?, 2000, 2000, 2001, 1)
    `).run(route.id, travelerVehicle, captorVehicle);

    const encounter = resolveNextVehicleEncounter(store);
    const report = store.battleReport(savedTraveler.id, encounter.battle_id);
    assert.equal(report.won, true);
    assert.deepEqual(report.details.result.captiveRounds.map((round) => round.round), [1, 2]);
    assert.deepEqual(report.details.result.captiveRounds.map(
      (round) => round.strikes[0].damage), [3, 3]);
    assert.deepEqual(report.details.result.freedCaptiveDwarves.map((entry) => entry.id),
      [stowawayId]);
    assert.equal(store.database.prepare(
      'SELECT status FROM dwarf_stowaways WHERE id = ?'
    ).get(stowawayId).status, 'escaped');
    assert.equal(store.database.prepare(
      'SELECT 1 FROM player_vehicles WHERE id = ?'
    ).get(captorVehicle), undefined);
    assert.match(store.recentMessages(savedTraveler.id, 'all', 'Vehicle')
      .find((message) => message.details?.battleId === encounter.battle_id).body,
    /Dwarf captured by the NPC fought back and escaped/u);
    assert.ok(travelerTrip.arrivesAt > 2001 && captorTrip.arrivesAt > 2001);
  });
...
test('grants registered miners a Council starter pack and kept sentencing message', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const playerState = createPlayer(
  ...
  const initialDwarfCount = Number(
    playerState.inventory[LEGACY_STARTER_WELCOME_PACK.dwarfItemId] ?? 0
  );
  const initialSafeTravelKitCount = Number(
    playerState.inventory[LEGACY_STARTER_WELCOME_PACK.safeTravelKitItemId] ?? 0
  );
  const initialGadgetCounts = new Map(LEGACY_STARTER_WELCOME_PACK.gadgetItemIds.map(
    (itemId) => [itemId, Number(playerState.inventory[itemId] ?? 0)]
  ));
  const initialSupplyCounts = new Map(LEGACY_STARTER_WELCOME_PACK.itemGrants.map(
    ({ itemId }) => [itemId, Number(playerState.inventory[itemId] ?? 0)]
  ));
  const player = store.addPlayer(playerState, {
  ...
  assert.equal(registered.inventory[LEGACY_STARTER_WELCOME_PACK.dwarfItemId],
    initialDwarfCount + 1);
  assert.equal(registered.inventory[LEGACY_STARTER_WELCOME_PACK.safeTravelKitItemId],
    initialSafeTravelKitCount + 1);
  for (const gadgetItemId of LEGACY_STARTER_WELCOME_PACK.gadgetItemIds) {
    assert.equal(registered.inventory[gadgetItemId],
      initialGadgetCounts.get(gadgetItemId) + 1);
  }
  for (const { itemId, quantity } of LEGACY_STARTER_WELCOME_PACK.itemGrants) {
    assert.equal(registered.inventory[itemId], initialSupplyCounts.get(itemId) + quantity);
  }
  ...
  const voucher = store.casinoState(player.id).currencies.find((currency) => currency.id === 1);
  assert.deepEqual({
    quantity: voucher.quantity,
    ownedQuantity: voucher.ownedQuantity,
    voucherQuantity: voucher.voucherQuantity
  }, { quantity: 105, ownedQuantity: 5, voucherQuantity: 100 });
  const messages = store.recentMessages(player.id, 'all', 'Admin');
  assert.equal(messages.length, 1);
  const [welcome] = messages;
  assert.equal(welcome.system, true);
  assert.equal(welcome.senderName, 'The Council');
  assert.equal(welcome.subject, 'Notice of banishment and estate disposition');
  assert.equal(welcome.details.event, 'registration-welcome');
  assert.equal(welcome.details.systemSenderName, 'The Council');
  assert.match(registered.docketNumber, /^MT-[A-F0-9]{12}$/u);
  assert.equal(player.docketNumber, registered.docketNumber);
  assert.equal(welcome.details.docket.docketId, registered.docketNumber);
  assert.match(welcome.body, new RegExp(`^Docket ${registered.docketNumber}$`, 'mu'));
  assert.ok(welcome.details.docket.charges.length >= 3
    && welcome.details.docket.charges.length <= 5);
  assert.ok(welcome.details.docket.charges.every((charge) => charge.includes(' while ')));
  assert.equal(typeof welcome.details.docket.sentence, 'string');
  assert.equal(welcome.read, false);
  assert.equal(welcome.deleted, false);
  assert.equal(welcome.kept, true);
  assert.equal(welcome.createdAt, 1000);
  const city = store.database.prepare('SELECT name FROM catalog_cities WHERE id = ?')
    .get(playerState.cityId);
  const mineType = store.database.prepare('SELECT name FROM catalog_mine_types WHERE id = ?')
    .get(playerState.mines[0].mineTypeId);
  assert.deepEqual(welcome.details.grants, {
    cityId: playerState.cityId, cityName: city.name,
    mineTypeId: playerState.mines[0].mineTypeId, mineTypeName: mineType.name,
    initialDiscoveries: playerState.discoveries.length,
    vehicleItemId: LEGACY_STARTER_WELCOME_PACK.vehicleItemId,
    vehicleName: 'Camel', vehicleColor: 'Yellow', vehicleQuantity: 1,
    dwarfItemId: LEGACY_STARTER_WELCOME_PACK.dwarfItemId,
    dwarfName: 'Green Dwarf', dwarfQuantity: 1,
    safeTravelKit: {
      itemId: SAFE_TRAVEL_KIT_CATALOG.itemId,
      itemName: 'Safe Travel Kit', itemRarity: 1, quantity: 1,
      contents: [
        { itemId: 2, itemName: 'Bolt', itemRarity: 1, quantity: 5 },
        { itemId: 1112, itemName: 'Tin Door Panels', itemRarity: 1, quantity: 1 },
        { itemId: 1116, itemName: 'Tin Quick Shift', itemRarity: 1, quantity: 1 }
      ]
    },
    gadgets: [
      { itemId: 255, itemName: 'Tin Shield', itemRarity: 1,
        behaviorKey: 'shield', displayName: 'Shield' },
      { itemId: 257, itemName: 'Tin Turbo Engine', itemRarity: 1,
        behaviorKey: 'turbo', displayName: 'Turbo Engine' },
      { itemId: 259, itemName: 'Tin Radar', itemRarity: 1,
        behaviorKey: 'radar', displayName: 'Radar' }
    ],
    supplies: [
      { itemId: 1278, itemName: 'Tin Pad', itemRarity: 1, quantity: 1,
        machineBehaviorKey: 'pad', explosive: false },
      { itemId: 1277, itemName: 'Tin Pump', itemRarity: 1, quantity: 1,
        machineBehaviorKey: 'pump', explosive: false },
      { itemId: 1272, itemName: 'Tin Pipe200', itemRarity: 1, quantity: 4,
        machineBehaviorKey: 'pipe200', explosive: false },
    ...
    rentalMines: rentalMines.filter((mine) =>
      ...
      mineTypeName: catalog.mineTypes.find((mineType) =>
        mineType.id === mine.mineTypeId).name,
      active: mine.active,
      rentalUntil: mine.rentalUntil
    })),
    rentalDurationMs,
    cryptoTypeId: LEGACY_STARTER_WELCOME_PACK.cryptoTypeId,
    cryptoName: 'Aso Coin', cryptoSymbol: 'ASO', cryptoQuantity: 5,
    casinoVoucherCryptoTypeId: 1, casinoVoucherCryptoName: 'Aso Coin',
    casinoVoucherCryptoSymbol: 'ASO', casinoVoucherQuantity: 100,
    credits: playerState.credits, gold: playerState.gold,
    batteryDurationMs: playerState.batteryExpiresAt - playerState.createdAt,
    itemLimit: playerState.itemLimit
  });
  assert.match(welcome.body, new RegExp(`1 x ${mineType.name} Mine in ${city.name}`));
  assert.match(welcome.body, /5 initial discoveries/);
  assert.match(welcome.body, /1 x Yellow Camel/);
  assert.match(welcome.body, /1 x Green Dwarf, already working the mines/);
  assert.match(welcome.body, /1 x Yellow Safe Travel Kit/);
  assert.match(welcome.body, /contains 5 x Bolt, 1 x Tin Door Panels, 1 x Tin Quick Shift/);
  assert.match(welcome.body, /1 x Yellow Tin Shield/);
  assert.match(welcome.body, /1 x Yellow Tin Turbo Engine/);
  assert.match(welcome.body, /1 x Yellow Tin Radar/);
  assert.match(welcome.body, /1 x Tin Pad/);
  assert.match(welcome.body, /1 x Tin Pump/);
  assert.match(welcome.body, /4 x Tin Pipe200 connectors/);
  assert.match(welcome.body, /5 x M-80 explosives/);
  assert.match(welcome.body, /1 x Equipment Mine rental/);
  assert.match(welcome.body, /1 x Vehicles Mine rental/);
  assert.match(welcome.body, /5 ASO \(Aso Coin\)/);
  assert.match(welcome.body, /100 ASO casino voucher/);
  assert.match(welcome.body, /100 credits and 5g/);
  assert.match(welcome.body, /20 hours of battery charge/);
  assert.match(welcome.body, /Capacity for 500 Things/);
  assert.match(welcome.body, /^THE COUNCIL/mu);
  assert.match(welcome.body, /You have been found guilty of the following offences:/u);
  assert.match(welcome.body, / while /u);
  assert.match(welcome.body, /Sentence proposed:/u);
  ...
  assert.match(welcome.body, /Seek out and clear Stones/u);
  assert.match(welcome.body, /surest way to increase your mining rate/u);
  assert.match(welcome.body, /Open your Safe Travel Kit/u);
  assert.match(welcome.body, /two Yellow defensive mods/u);
  assert.deepEqual(welcome.details.actions, [
    { label: 'Check your mine', path: '/' },
    { label: 'Create your starter bot', path: '/' },
    { label: 'Seek out Stones', path: '/stones' },
    ...
    { label: 'Open your Safe Travel Kit', path: '/inventory' },
    { label: 'Activate your Camel', path: '/vehicles' },
    { label: 'Spend your ASO voucher', path: '/casino' },
    { label: 'Open the world map', path: '/map' },
    { label: 'Read the Field guide', path: '/guide' }
  ]);
  assert.deepEqual(welcome.actionLinks, welcome.details.actions.map((action) => ({
    label: action.label, href: action.path
  })));
  const rawWelcome = store.database.prepare(`
    SELECT dedupe_key, is_read, is_deleted, is_kept FROM messages WHERE id = ?
  `).get(welcome.id);
  assert.deepEqual({ ...rawWelcome }, {
    dedupe_key: 'registration-welcome:v2', is_read: 0, is_deleted: 0, is_kept: 1
  });
  const retention = JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'message_retention_ms'
  `).get().value_json);
  assert.equal(store.expireMessages(1000 + retention + 1), 0);
  assert.equal(store.recentMessages(player.id, 'all', 'Admin').length, 1);
  const prompt = store.registrationWelcomePrompt(player.id);
  assert.equal(prompt.id, welcome.id);
  assert.equal(prompt.senderName, 'The Council');
  store.ignoreRegistrationWelcome(player.id, welcome.id);
  assert.equal(store.registrationWelcomePrompt(player.id), null);
  assert.equal(store.recentMessages(player.id, 'unread', 'Admin').length, 1);
  assert.equal(store.recentMessages(player.id, 'all', 'Admin')[0].kept, true);
  assert.equal(store.recentMessages(player.id, 'all', 'Admin')[0].read, false);
...
test('uses live starter-pack data in registration welcome messages', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const landRouteType = Number(catalog.settings.route_type_ids.land);
  const alternativeVehicle = store.database.prepare(`
    SELECT catalog_items.id, catalog_items.name
    FROM catalog_items
    JOIN catalog_vehicles ON catalog_vehicles.item_id = catalog_items.id
    WHERE catalog_items.rarity = 1 AND catalog_vehicles.route_type = ?
      AND catalog_items.id <> ?
    ORDER BY catalog_items.id LIMIT 1
  `).get(landRouteType, LEGACY_STARTER_WELCOME_PACK.vehicleItemId);
  assert.ok(alternativeVehicle);
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = ? WHERE key = 'starter_welcome_pack'
  `).run(JSON.stringify({
    ...LEGACY_STARTER_WELCOME_PACK,
    vehicleItemId: alternativeVehicle.id,
    cryptoTypeId: 2,
    cryptoQuantity: 7
  }));
  const player = store.addPlayer(createPlayer(
    'Live Pack Miner', 'live-pack@example.test', 'hash', catalog, 2000, () => 0.5
  ), { version: '2026-08-28', acceptedAt: 2000 });
  const welcome = store.recentMessages(player.id, 'all', 'Admin')[0];

  assert.equal(welcome.details.grants.vehicleItemId, alternativeVehicle.id);
  assert.equal(welcome.details.grants.vehicleName, alternativeVehicle.name);
  assert.equal(welcome.details.grants.dwarfItemId, LEGACY_STARTER_WELCOME_PACK.dwarfItemId);
  assert.equal(welcome.details.grants.dwarfName, 'Green Dwarf');
  assert.deepEqual(welcome.details.grants.gadgets.map((gadget) => gadget.itemId),
    LEGACY_STARTER_WELCOME_PACK.gadgetItemIds);
  assert.equal(welcome.details.grants.cryptoTypeId, 2);
  assert.equal(welcome.details.grants.cryptoSymbol, 'BRO');
  assert.equal(welcome.details.grants.cryptoQuantity, 7);
  assert.equal(welcome.details.grants.casinoVoucherCryptoSymbol, 'ASO');
  assert.equal(welcome.details.grants.casinoVoucherQuantity, 100);
  assert.match(welcome.body, new RegExp(`1 x Yellow ${alternativeVehicle.name}`));
  assert.match(welcome.body, /1 x Green Dwarf/);
  assert.match(welcome.body, /1 x Yellow Tin Shield/);
  assert.match(welcome.body, /7 BRO \(Bromo Byte\)/);
  assert.equal(store.playerById(player.id).cryptoBalances[2], 7);
  assert.equal(store.playerById(player.id).inventory[LEGACY_STARTER_WELCOME_PACK.dwarfItemId], 1);
});

test('backfills durable unique Council docket numbers and preserves existing notices', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-docket-migration-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    store.close();
  ...
  store.seedCatalog(catalog);
  const first = store.addPlayer(createPlayer(
    'Legacy Docket', 'legacy-docket@example.test', 'hash', catalog, 1000, () => 0.5
  ), { version: '2026-08-28', acceptedAt: 1000 });
  const second = store.addPlayer(createPlayer(
    'No Notice Docket', 'no-notice-docket@example.test', 'hash', catalog, 1001, () => 0.5
  ));
  const welcomeRow = store.database.prepare(`
    SELECT id, body, details_json FROM messages
    WHERE recipient_id = ? AND dedupe_key = 'registration-welcome:v2'
  `).get(first.id);
  const details = JSON.parse(welcomeRow.details_json);
  details.docket.docketId = 'ABCDEF123456';
  store.database.exec(`
    DROP TRIGGER players_assign_docket_number;
    DROP TRIGGER players_preserve_docket_number;
    DROP INDEX players_docket_number_unique;
  `);
  store.database.prepare(
    'UPDATE messages SET body = ?, details_json = ? WHERE id = ?'
  ).run(welcomeRow.body.replace(/^Docket [^\r\n]+/mu, 'Docket ABCDEF123456'),
    JSON.stringify(details), welcomeRow.id);
  store.database.prepare('UPDATE players SET docket_number = NULL').run();
  store.database.exec('PRAGMA user_version = 114');
  store.close();

  store = new SqliteStore(databaseFile);
  const migratedFirst = store.playerById(first.id);
  const migratedSecond = store.playerById(second.id);
  const migratedWelcome = store.recentMessages(first.id, 'all', 'Admin').find(
    (message) => message.details.event === 'registration-welcome'
  );
  assert.ok(migratedWelcome);
  assert.equal(migratedFirst.docketNumber, 'ABCDEF123456');
  assert.equal(migratedWelcome.details.docket.docketId, migratedFirst.docketNumber);
  assert.match(migratedWelcome.body, /^Docket ABCDEF123456$/mu);
  assert.match(migratedSecond.docketNumber, /^MT-[A-F0-9]{12}$/u);
  assert.notEqual(migratedSecond.docketNumber, migratedFirst.docketNumber);
  assert.throws(() => store.database.prepare(
    'UPDATE players SET docket_number = ? WHERE id = ?'
  ).run('REASSIGNED', first.id), /immutable/u);

  const duplicate = createPlayer(
    'Duplicate Docket', 'duplicate-docket@example.test', 'hash', catalog, 1002, () => 0.5
  );
...
test('rolls back registration and grants when its welcome message cannot be created', (context) => {
  ...
  store.seedCatalog(catalog);
  store.database.exec(`
    CREATE TRIGGER reject_registration_welcome
    BEFORE INSERT ON messages
    WHEN NEW.dedupe_key = 'registration-welcome:v2'
    BEGIN
      SELECT RAISE(ABORT, 'forced welcome failure');
    END;
  `);
  const playerState = createPlayer(
    'Rolled Back Welcome', 'rollback-welcome@example.test', 'hash', catalog, 3000,
    () => 0.5
  );

  assert.throws(() => store.addPlayer(playerState, {
    version: '2026-08-28', acceptedAt: 3000
  }), /forced welcome failure/);
  assert.equal(store.database.prepare(
...
test('records credit purchases idempotently and reverses the granted bundle once', (context) => {
  ...
  const messages = store.recentMessages(player.id, 'all', 'Admin');
  const creditMessages = messages.filter((message) =>
    String(message.details.event).startsWith('credit-purchase-'));
  assert.deepEqual(creditMessages.map((message) => message.details.event).sort(),
...
test('settles casino pulls atomically in gold or crypto and preserves the machine ledger', (context) => {
  ...
  const pairMigration = store.database.prepare(`
    SELECT details_json FROM schema_migrations WHERE name = 'casino-explosive-pairs-v4'
  `).get();
  assert.ok(pairMigration);
  assert.deepEqual(JSON.parse(pairMigration.details_json), {
    version: 4,
    explosiveSmallMatchMultiplier: 1
  });
  const minimumMigration = store.database.prepare(`
    SELECT details_json FROM schema_migrations WHERE name = 'casino-explosive-minimum-v5'
  `).get();
  assert.ok(minimumMigration);
  assert.deepEqual(JSON.parse(minimumMigration.details_json), {
    version: 5,
    explosiveSmallMatchMinimum: 3
  });
});

test('spends casino vouchers before owned ASO and pays winnings into the real balance', (context) => {
  const store = new SqliteStore(':memory:');
  context.after(() => store.close());
  store.seedCatalog(catalog);
  const player = store.addPlayer(createPlayer(
    'Voucher Miner', '', 'hash', catalog, 1000, () => 0.5
  ));
  store.database.prepare(`
  ...
  const initial = store.casinoState(player.id).currencies.find((currency) => currency.id === 1);
  assert.deepEqual({
    quantity: initial.quantity,
    ownedQuantity: initial.ownedQuantity,
    voucherQuantity: initial.voucherQuantity
  }, { quantity: 100, ownedQuantity: 0, voucherQuantity: 100 });
  store.spinCasino(player.id, 'crypto:1', 12, sequence([
    0.5, ...Array.from({ length: 9 }, (_, index) => (index + 0.1) / 12)
  ]), 2000);
  const afterLoss = store.casinoState(player.id).currencies.find((currency) => currency.id === 1);
  assert.deepEqual({
    quantity: afterLoss.quantity,
    ownedQuantity: afterLoss.ownedQuantity,
    voucherQuantity: afterLoss.voucherQuantity
  }, { quantity: 88, ownedQuantity: 0, voucherQuantity: 88 });

  const win = store.spinCasino(
    player.id, 'crypto:1', 2, sequence([0.5, ...Array(9).fill(0)]), 3000
  );
  assert.equal(win.payout, 32);
  const afterWin = store.casinoState(player.id).currencies.find((currency) => currency.id === 1);
  assert.deepEqual({
    quantity: afterWin.quantity,
    ownedQuantity: afterWin.ownedQuantity,
    voucherQuantity: afterWin.voucherQuantity
  }, { quantity: 118, ownedQuantity: 32, voucherQuantity: 86 });
  assert.equal(store.playerById(player.id).cryptoBalances[1], 32);
});

test('v4 and v5 casino explosive migrations preserve unrelated live rule tuning', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-casino-v5-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    try { store.close(); } catch {}
  ...
  const setting = store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'casino_slot_rules'
  `).get();
  const legacyRules = JSON.parse(setting.value_json);
  legacyRules.version = 3;
  delete legacyRules.explosiveSmallMatchMultiplier;
  delete legacyRules.explosiveSmallMatchMinimum;
  delete legacyRules.explosiveScatterCountFactors['2'];
  legacyRules.maximumGoldWager = 777;
  legacyRules.explosiveScatterCountFactors['5'] = 7;
  store.database.prepare(`
    UPDATE catalog_settings SET value_json = ? WHERE key = 'casino_slot_rules'
  `).run(JSON.stringify(legacyRules));
  ...
  for (const migrationName of [
    'casino-slot-v1', 'casino-explosive-scatters-v2', 'casino-bonus-respins-v3'
  ]) markApplied.run(migrationName);

  store.close();
  store = new SqliteStore(databaseFile);

  const migratedRules = JSON.parse(store.database.prepare(`
    SELECT value_json FROM catalog_settings WHERE key = 'casino_slot_rules'
  `).get().value_json);
  assert.equal(migratedRules.version, 5);
  assert.equal(migratedRules.explosiveSmallMatchMultiplier, 1);
  assert.equal(migratedRules.explosiveSmallMatchMinimum, 3);
  assert.equal(migratedRules.maximumGoldWager, 777,
    'an unrelated live wager limit is preserved');
  assert.equal(migratedRules.explosiveScatterCountFactors['5'], 7,
    'an unrelated live scatter factor is preserved');
  assert.equal(Object.hasOwn(migratedRules.explosiveScatterCountFactors, '2'), false,
    'the additive migration does not rewrite the existing scatter table');
  const marker = store.database.prepare(`
    SELECT details_json FROM schema_migrations WHERE name = 'casino-explosive-pairs-v4'
  `).get();
  assert.deepEqual(JSON.parse(marker.details_json), {
    version: 4,
    explosiveSmallMatchMultiplier: 1
  });
  const minimumMarker = store.database.prepare(`
    SELECT details_json FROM schema_migrations WHERE name = 'casino-explosive-minimum-v5'
  `).get();
  assert.deepEqual(JSON.parse(minimumMarker.details_json), {
    version: 5,
    explosiveSmallMatchMinimum: 3
  });
});

test('retains privacy-limited shill signals and scores corroborating account evidence',
  (context) => {
    const store = new SqliteStore(':memory:');
    context.after(() => store.close());
    store.seedCatalog(catalog);
    store.ensureWorldMaps(1000);
    const currentTime = 50 * 86400000;
    const first = store.addPlayer(createPlayer(
      'Signal One', 'one@example.com', 'hash', catalog, currentTime - 7200000, () => 0.5
    ));
    const second = store.addPlayer(createPlayer(
      'Signal Two', 'two@example.com', 'hash', catalog, currentTime - 3600000, () => 0.5
    ));
    const unrelated = store.addPlayer(createPlayer(
      'Signal Three', 'three@example.com', 'hash', catalog, currentTime - 10 * 86400000,
      () => 0.5
    ));
    const sharedToken = 'a'.repeat(64);
    store.recordAccountNetworkObservation(unrelated.id, 'b'.repeat(64), 'local',
      currentTime - 31 * 86400000);
    store.recordAccountNetworkObservation(first.id, sharedToken, 'local', currentTime - 1000);
    store.recordAccountNetworkObservation(second.id, sharedToken, 'google', currentTime - 500);

    const item = store.database.prepare(`
      SELECT id, gold_value_units FROM catalog_items
      WHERE gold_value_units > 0 ORDER BY gold_value_units, id LIMIT 1
    `).get();
    const insertSale = store.database.prepare(`
      ...
      VALUES (?, ?, 1, ?, ?, 1, ?)
    `);
    const suspiciousPriceUnits = Math.max(item.gold_value_units * 10, 2000 * 10000);
    insertSale.run(first.id, second.id, item.id, suspiciousPriceUnits, currentTime - 400);
    insertSale.run(second.id, first.id, item.id, suspiciousPriceUnits, currentTime - 300);

    const signals = store.adminShillSignals(currentTime);
    assert.equal(signals.networkObservationCount, 2,
      'an expired observation is pruned on the next successful sign-in');
    const pair = signals.pairs.find((entry) =>
      entry.firstId === Math.min(first.id, second.id)
      && entry.secondId === Math.max(first.id, second.id));
    assert.ok(pair);
    assert.equal(pair.sharedNetworkCount, 1);
    assert.equal(pair.tradeCount, 2);
    assert.equal(pair.directions.length, 2);
    assert.equal(pair.level, 'high');
    assert.ok(pair.score >= 70);
    assert.ok(pair.evidence.some((entry) => entry.includes('Same public sign-in address')));
    assert.ok(pair.evidence.some((entry) => entry.includes('fixed value')));
    assert.equal(pair.trades.length, 2);
    assert.equal(store.adminShillSignals(currentTime, { playerId: unrelated.id }).pairs.length, 0);
    assert.throws(() => store.recordAccountNetworkObservation(
      first.id, 'not-a-token', 'local', currentTime
    ), /Invalid network evidence token/);
    assert.throws(() => store.recordAccountNetworkObservation(
      first.id, 'c'.repeat(64), 'other', currentTime
    ), /Invalid authentication method/);
  });

test('migrates existing expedition messages into ranked rewards and Worldwire', (context) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-expedition-migration-'));
  const databaseFile = path.join(directory, 'game.sqlite');
  let store = new SqliteStore(databaseFile);
  context.after(() => {
    try { store.close(); } catch {}
  ...
  store.ensureWorldMaps(1000);
  const player = store.addPlayer(createPlayer(
    'Historical Pioneer', '', 'hash', store.loadCatalog(), 1000, () => 0.5
  ));
  const bromo = store.database.prepare(
    "SELECT id, slug FROM world_maps WHERE name = 'Bromo'"
  ).get();
  const item = store.database.prepare(`
    SELECT catalog_items.id, catalog_items.name
    FROM catalog_items
    JOIN catalog_city_mine_types
      ON catalog_city_mine_types.mine_type_id = catalog_items.mine_type_id
    JOIN catalog_cities ON catalog_cities.id = catalog_city_mine_types.city_id
    WHERE catalog_cities.map_id = ? AND catalog_items.can_find = 1
      AND catalog_items.repaired_item_id IS NULL
    ORDER BY catalog_items.rarity DESC, catalog_items.id LIMIT 1
  `).get(bromo.id);
  store.database.prepare(`
    INSERT INTO messages
      (recipient_id, message_type, subject, body, details_json, dedupe_key, created_at)
    VALUES (?, 'City', 'Bromo expedition reward', 'Historical reward', ?, ?, 2000)
  `).run(player.id, JSON.stringify({
    event: 'map-discovered', mapId: bromo.id, mapName: 'Bromo',
    credits: 500, itemId: item.id
  }), `map:${bromo.id}:first-arrival:${player.id}`);
  store.database.exec(`
    DROP TABLE regional_expedition_rewards;
    DELETE FROM catalog_settings WHERE key = 'inter_map_expedition_credit_awards';
    DELETE FROM world_chat_announcements WHERE event_key LIKE 'frontier-expedition:%';
  ...
  assert.deepEqual(JSON.parse(store.database.prepare(`
    ...
    WHERE key = 'inter_map_expedition_credit_awards'
  `).get().value_json), [500, 400, 300, 200, 100]);
  const reward = store.database.prepare(`
    SELECT * FROM regional_expedition_rewards WHERE map_id = ?
  `).get(bromo.id);
  assert.deepEqual({
    position: reward.position, playerId: reward.player_id,
    playerName: reward.player_name, credits: reward.credits,
    itemId: reward.item_id, itemName: reward.item_name
  }, {
    position: 1, playerId: player.id, playerName: player.name,
    credits: 500, itemId: item.id, itemName: item.name
  });
  const announcement = store.recentChats().find((chat) =>
    chat.eventKey === `frontier-expedition:${bromo.id}:1`);
  assert.ok(announcement);
  assert.match(announcement.body, /Historical Pioneer/u);
  assert.match(announcement.body, /Bromo/u);
  assert.equal(announcement.path, `/map?world=${bromo.slug}`);
});

test('migrates a version 136 database to pseudonymous account sign-in observations',
  (context) => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-shill-migration-'));
    const databaseFile = path.join(directory, 'game.sqlite');
    let store = new SqliteStore(databaseFile);
    context.after(() => {
      try { store.close(); } catch {}
    ...
    store.close();
    const legacy = new DatabaseSync(databaseFile);
    legacy.exec('DROP TABLE account_network_observations; PRAGMA user_version = 136;');
    legacy.close();
...
test('migration 138 discards address observations collected with the unsafe proxy fallback',
  (context) => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'minethings-shill-reset-'));
    const databaseFile = path.join(directory, 'game.sqlite');
    let store = new SqliteStore(databaseFile);
    context.after(() => {
      try { store.close(); } catch {}
    ...
    store.ensureWorldMaps(1000);
    const player = store.addPlayer(createPlayer(
      'Signal Reset', 'reset@example.com', 'hash', catalog, 1000, () => 0.5
    ));
    store.recordAccountNetworkObservation(player.id, 'd'.repeat(64), 'local', 2000);
    store.close();
    const legacy = new DatabaseSync(databaseFile);
    legacy.exec('PRAGMA user_version = 137;');
    legacy.close();
