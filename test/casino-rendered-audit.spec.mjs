...
const machines = [
  { slug: 'thing-o-matic', path: '/casino' },
  { slug: 'bromo-sporefall', path: '/casino?machine=bromo-sporefall' },
  { slug: 'kings-lockbox', path: '/casino?machine=kings-lockbox' }
];
const viewports = [
...
test.beforeAll(async () => {
  ...
  store.addPlayer(createPlayer(
    'CasinoAudit', '', hashPassword(password), catalog, 1000, () => 0.5
  ));
  store.close();
...
async function login(page) {
  ...
  await page.locator('form[action="/login"] input[name="name"]').fill('CasinoAudit');
  ...
  for (const viewport of viewports) {
    ...
    for (const machine of machines) {
      await test.step(`${machine.slug} at ${viewport.name} size`, async () => {
        ...
        const casinoPage = page.locator(`.casino-page.casino-theme-${machine.slug}`);
        ...
        await expect(cabinet.locator('[data-casino-cell]')).toHaveCount(9);
        await expect(picker).toHaveCount(1);
        await expect(pickerLinks).toHaveCount(3);
